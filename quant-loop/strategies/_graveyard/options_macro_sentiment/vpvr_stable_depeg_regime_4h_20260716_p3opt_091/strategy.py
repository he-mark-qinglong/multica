"""vpvr_stable_depeg_regime_4h_20260716_p3opt_091 — strategy backtest engine.

P3-OPT-091 variant: introduces signal_blend_weight param (0.5) which blends
primary VPVR POC reversion with secondary momentum confirmation. Otherwise
identical to baseline vpvr_stable_depeg_regime_4h_20260716.
...
VARIANT_KEY = "vpvr_stable_depeg_regime_4h_20260716_p3opt_091"
...
def run_backtest(df: pd.DataFrame, cfg: dict) -> dict:
    """Run single-symbol backtest on a DataFrame with OHLCV + premium columns."""
    p = cfg["params"]
    ...
    regime_ok = sig_df["regime_ok"].astype(bool)
    premium = sig_df["premium"].astype(np.float64)
    poc_dist = sig_df["poc_distance_atr"].fillna(np.inf)

    close_arr = close.values
    atr_arr = atr.values
    sig_arr = signal.values
    regime_arr = regime_ok.values
    premium_arr = premium.values
    ...
    warmup = max(p["vpvr_window_bars"], p["atr_period"]) + 1
    ...
    for i in range(1, len(df)):
        ...
        sig = int(sig_arr[i])
        regime = bool(regime_arr[i])

        if pos == 0:
            bars_since_exit += 1
            if (bars_since_exit >= p["cooldown_bars"]
                    and sig != 0
                    and regime
                    and at > 0):
                pos = sig
                entry_idx = i
                entry_px = px
                entry_premium = float(premium_arr[i])
            ...
            if not regime:
                exit_now = True
                exit_reason = "depeg_exit"
            elif move >= p["take_profit_atr_k"] * (at / entry_px):
                exit_now = True
            ...
            if exit_now:
                ...
                trades.append(Trade(
                    ...
                    premium_at_entry=entry_premium,
