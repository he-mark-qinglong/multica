...
def main() -> int:
    ...
    live_p = sub.add_parser("live",
                             help="connect Binance public WS and run fill-engine")
    live_p.add_argument("--window-min", type=int, default=30,
                         help="bounded runtime in minutes (default 30)")
    live_p.add_argument("--smark-max-dd", type=float, default=5.0,
                         help="absolute maxDD %% halt threshold (smark 2026-07-20)")
    live_p.add_argument("--smark-daily-loss", type=float, default=2.0,
                         help="absolute daily_loss %% halt threshold (smark 2026-07-20)")
    ...
    if args.cmd == "live":
        cfg = _load_config(args.config)
        from fill_engine import run_live  # lazy import to keep init/scaffold fast
        return run_live(cfg, window_min=args.window_min,
                        smark_absolute_max_dd=args.smark_max_dd,
                        smark_absolute_daily_loss=args.smark_daily_loss)
    return 0
