"""V3 strategy (xs-pair z-score + VPVR confluence, iter#75).
...
Entry:
  ...
  - Confluence: BTC's close must lie within ``proximity_atr_k * ATR(14)`` of
    BTC's rolling VPVR POC over the most recent ``vpvr_window_bars``.
...
Returns
-------
A dict result with keys: trades, equity, bar_return, positions, signal_a, signal_b,
                        per_pair_metrics.
"""
...
def _rolling_vpvr(close: np.ndarray, volume: np.ndarray, window: int, n_bins: int) -> dict:
    ...
    `poc[i]` is the POC of bars `[i - window, i)` — bars up to but NOT
    INCLUDING bar `i`. This makes the signal usable at bar `i` without
    peeking at `close[i]`.

    Warmup: bars [0, window) → NaN (insufficient history).
    """
    n = len(close)
    poc = np.full(n, np.nan)
    for i in range(window, n):
        # Strictly past window: bars [i - window, i).
...
def run_pair_backtest(df_a: pd.DataFrame, df_b: pd.DataFrame, cfg: dict, pair_label: str) -> dict:
    ...
    trade_log = []
    pos = 0  # 0 flat, +1 "long_a_short_b", -1 "short_a_long_b"
    bars_held = 0
    entry_idx = None
    entry_z = None
    entry_a = None
    ...
    for i in range(1, n):
        ...
        if cur_pos == 0 and zi is not None:
            if zi >= entry_thr:
                cur_pos = -1  # short A, long B
            elif zi <= -entry_thr:
                cur_pos = +1  # long A, short B
        if cur_pos != 0 and pos == 0:
            ...
            bars_held += 1
            # Mark intra-bar return: A long-short contribution * (A ret - B ret)
            a_ret = float(a["close"].iat[i]) / float(a["close"].iat[i - 1]) - 1.0
            b_ret = float(b["close"].iat[i]) / float(b["close"].iat[i - 1]) - 1.0
            pnl_pct_per_bar[i] = pos * (a_ret - b_ret) / 2.0  # symmetric scaling
            exit_reason = None
            # Confluence-loss exit: if position is held and confluence
            # requirement (close near VPVR POC) is no longer satisfied,
            # close the trade. Only when entry-gate required confluence.
            ...
            # Forced close: confluence lost mid-holding.
            exit_a = float(a["close"].iat[i])
            exit_b = float(b["close"].iat[i])
            if pos == +1:
                pct = (exit_a / entry_a - 1.0) - (exit_b / entry_b - 1.0)
    ...
    # Build equity for this pair (averaged across both legs, no leverage).
    ...
    # Distribute per-trade cost evenly across holding window (round-trip cost was already deducted net-of-trade; here we amend for a flat per-bar cost model)
    # Already accounted at trade-level; bar-level just gets the price diffs.
...
def run_backtest(data: dict, cfg: dict) -> dict:
    ...
    ``data`` is a dict of symbol -> DataFrame (already on the target timeframe).
    ...
    # Combined portfolio: average equity across pairs.
