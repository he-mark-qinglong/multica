"""vpvr_macro_calendar_4h_20260715 — strategy backtest engine.
...
VARIANT_KEY = "vpvr_macro_calendar_4h_20260715"
...
class Trade:
    ...
    bars_held: int
    exit_reason: str
    macro_proximity_at_entry: int
    poc_distance_atr_at_entry: float


def run_backtest(df: pd.DataFrame, cfg: dict) -> dict:
    """Run single-symbol backtest on a DataFrame with OHLCV columns."""
    p = cfg["params"]
    ...
    proximity = sig_df["macro_proximity_bars"].astype(np.int64)
    poc_dist = sig_df["poc_distance_atr"].astype(np.float64)
    ...
    warmup = max(p["vpvr_window_bars"], p["atr_period"] * 5) + 1
    ...
    for i in range(1, len(df)):
        ...
        if pos == 0:
            ...
            if (bars_since_exit >= p["cooldown_bars"]
                ...
                entry_idx = i
                entry_px = px
                entry_proximity = int(proximity.iloc[i])
            ...
            if exit_now:
                ...
                trades.append(Trade(
                    ...
                    macro_proximity_at_entry=entry_proximity,
