"""Freqtrade framework adapter for vpvr_stable_depeg_regime_4h_20260716.
...
Replay notes:
  ...
  - 1m data spans 2019-09-08 → 2026-07-17; the strategy trades file
    extends to 2027-07-19 (synthetic forward tail). Replay uses only the
    97 / 123 trades whose entry AND exit fall within the 1m data window
    (subset documented in `framework_cv_freqtrade.json`).
  - pnl_pct applied linearly across held 4h bars with weight = risk_target_pct
    = 0.005 from config.json.
...
try:
    ...
    class V72StableDepegRegime4hFreqtradeStrategy(IStrategy):
        """Freqtrade IStrategy wrapper for vpvr_stable_depeg_regime_4h_20260716."""
        timeframe = "4h"
    ...
    class V72StableDepegRegime4hFreqtradeStrategy(IStrategy):  # type: ignore[no-redef]
        def __init__(self, config: dict) -> None:
            self.config = config
            self.position = {"direction": "flat", "entry_ts": None,
                             "entry_price": 0.0, "stop": 0.0, "tp": 0.0,
