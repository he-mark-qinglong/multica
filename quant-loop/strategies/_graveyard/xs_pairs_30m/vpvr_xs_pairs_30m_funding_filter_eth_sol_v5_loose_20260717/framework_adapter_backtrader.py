"""Backtrader framework adapter for vpvr_xs_pairs_30m_funding_filter_eth_sol_v5_loose_20260717.
...
V5 loose — original V5 settings, NOT-PROFITABLE in-house per
Sharpe 0.422 < 0.5 gate). Both legs are native 30m parquets (no
15m resample).
...
pair-spread series (ETHUSDT - SOLUSDT * ratio_at_t0, normalised)
to verify the calibrated replay.

The in-house convention mirrored here:
  - Per-bar GROSS mark on close-to-close:
  ...
  - In-house cost = 8bp pair round-trip
    (1bp fee + 1bp slip per side per leg × 4 fills;
    matches config.json fees_bps_per_side=1.0 /
    slippage_bps_per_side=1.0).
  - Backtrader cost schedule = 24bp pair round-trip
    (3bp fee + 3bp slip per side per leg × 4 fills).
  - Framework replay uses INHOUSE_COST_RT_PAIR = 0.0008 directly so
    the comparison isolates equity-walk mechanics from cost-model
    delta.

Data handling mirrors `data_loader.py`:
  - ETHUSDT native 30m parquet (open_time ms column → DatetimeIndex).
  - SOLUSDT native 30m parquet (open_time ms column → DatetimeIndex).
  - ETHUSDT funding + SOLUSDT funding parquets exist but the trade
...
2024-07. walk_forward.json is NOT produced for iter#85 V5 loose
(same situation as iter#84 V4 variant; OOS reference metrics come
from the in-house aggregated metrics.json, exactly matching the
...
# Framework replay is calibrated to in-house cost (8bp) so the comparison
# isolates equity-walk mechanics from cost-model delta. The backtrader 24bp
# schedule is calibrated to in-house 8bp basis; the +16bp delta is reported
# in `notes` for visibility but is NOT applied to the comparison replay.
...
# real backtrader import succeeded (we still define a shim so the rest of
# the file can reference the class).
if _HAS_BACKTRADER:
    class VPVRXsPairs30mETHSOLV5LooseBacktraderStrategy(bt.Strategy):
        """Backtrader IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_eth_sol_v5_loose_20260717.
    ...
    class VPVRXsPairs30mETHSOLV5LooseBacktraderStrategy:  # type: ignore[no-redef]
...
def replay_backtrader_bar_mtm(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    cost schedule (calibrated to in-house 8bp here for apples-to-apples
    comparison).
...
def _run_backtrader_engine(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    cerebro.addstrategy(
        VPVRXsPairs30mETHSOLV5LooseBacktraderStrategy,
...
def main() -> int:
    ...
    # ---- Load native 30m parquets for both legs
    ...
    n_prices = len(prices)
    print(f"  common bars (post native 30m intersection): {n_prices} "
          f"({common[0]} → {common[-1]})")
    print(f"  ETHUSDT 30m native rows: {len(eth)} (intersection)")
    print(f"  SOLUSDT 30m native rows: {len(sol)} (intersection)")
    ...
    validation = {
        ...
        "note": (
            ...
            "(INHOUSE_COST_RT_PAIR, 1bp fee + 1bp slip per side per leg × 4 "
            "fills, matches config.json fees_bps_per_side=1.0 / "
            "slippage_bps_per_side=1.0). Validation reproduces this exactly "
            "by replaying trades with cost_rt=INHOUSE_COST_RT_PAIR. "
            "ETH and SOL close series are native 30m parquets "
            "(open_time ms column → DatetimeIndex)."
        ),
    }
    ...
    cv_record = {
        ...
        "inhouse_oos_walkforward": {
            ...
            "note": (
                "from walk_forward.json aggregate (mean_test_sharpe/return/"
                "worst_test_mdd) when available; otherwise single in-house "
                "aggregated metrics as OOS proxy."
        ...
        "max_abs_rel_divergence_pct": max_abs_rel_div_pct,
        "w5_threshold_pct": W5_THRESHOLD,
        "w5_auto_archive": auto_archive,
        "w5_tipping_metrics": [k for k, v in [
            ("oos_sharpe", div_sharpe),
            ("oos_ann_total_return", div_ann),
            ("oos_max_dd", div_mdd),
        ] if v > W5_THRESHOLD],
        "w5_verdict": "AUTO-ARCHIVE" if auto_archive else "WITHIN_TOLERANCE",
        "backtrader_imported": bool(_HAS_BACKTRADER),
        "notes": [
            "Pair strategy (ETHUSDT/SOLUSDT 30m): bar-by-bar MTM with `pos * (a_ret - b_ret) / 2.0`.",
            "ETH and SOL loaded as native 30m parquets (open_time ms column → DatetimeIndex).",
            "In-house cost = 8bp pair round-trip (1bp fee + 1bp slip per side per leg × 4 fills, per config.json).",
            "Backtrader cost schedule = 24bp pair round-trip (3bp fee + 3bp slip per side per leg × 4 fills).",
            f"INHOUSE_COST_RT_PAIR={INHOUSE_COST_RT_PAIR:.4f} (8bp pair round-trip, matches config.json).",
            f"BACKTRADER_COST_RT_PAIR={BACKTRADER_COST_RT_PAIR:.4f} (24bp pair round-trip, full backtrader schedule).",
            "Framework replay calibrated to in-house cost (8bp) so the comparison isolates equity-walk mechanics; +16bp conservative stress delta is reported for visibility but NOT applied to the comparison replay.",
            "Real backtrader engine was run on a normalized pair-spread synthetic series to verify the calibrated replay.",
            "iter#85 V5 loose ETH/SOL variant (original V5 settings, no regularization); in-house tag=NOT-PROFITABLE (sharpe -6.40 / total_return -98.57% / max_dd -98.59%, 5642 trades).",
            "V5 loose parameters: entry_z=2.0, lookback=96, max_hold=48, fund_thr=0.0005, vpvr_proximity_atr_k=0.7.",
