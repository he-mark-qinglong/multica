"""vpvr_defi_basis_15m_hyperliquid_dydx_20260716 — strategy backtest engine.
...
VARIANT_KEY = "vpvr_defi_basis_15m_hyperliquid_dydx_20260716"
...
class Trade:
    ...
    bars_held: int
    exit_reason: str
    basis_z_at_entry: float
    poc_distance_atr_at_entry: float


def run_backtest(df: pd.DataFrame, cfg: dict) -> dict:
    """Run single-symbol backtest on a DataFrame with OHLCV + basis columns."""
    p = cfg["params"]
    ...
    warmup = max(p["vpvr_window_bars"], p["basis_z_lookback_bars"], p["atr_period"]) + 1
    ...
    for i in range(1, len(df)):
        ...
        if pos == 0:
            ...
            if bars_since_exit >= p["cooldown_bars"] and sig != 0 and at > 0:
                ...
                entry_idx = i
                entry_px = px
                entry_bz = float(bz_arr[i]) if np.isfinite(bz_arr[i]) else 0.0
