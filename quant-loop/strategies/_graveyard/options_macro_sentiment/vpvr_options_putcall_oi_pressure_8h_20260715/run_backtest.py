"""Run a self-contained backtest for vpvr_options_putcall_oi_pressure_8h_20260715.

B3-spec implementation: loads real Binance USD-M 30m ETHUSDT klines
(`/home/smark/multica/quant-loop/data/perp_30m/ETHUSDT_30m.parquet`), aggregates
them to 8h bars (the row count matches the data/manifest.txt 4956 expectation),
constructs the taker_buy_share put-call pressure proxy, runs the strategy, and
writes the canonical evidence set:
  ...
  - results/trades_A_<TF>_<SYM>.csv  (>= 4 rows for display-engine variant detection)
  - results/equity_<SYM>.csv        (per-symbol equity curve)

The strategy logic, fee model, ATR-based exits, and warmup definition are
identical in shape to the VPVR family. The proxy itself is deterministic
(taker_buy_quote / quote_volume aggregated to 8h).
"""
...
SOURCE_PARQUET = Path("/home/smark/multica/quant-loop/data/perp_30m/ETHUSDT_30m.parquet")


def _load_8h_ohlcv_with_pcr_proxy(parquet_path: Path = SOURCE_PARQUET) -> pd.DataFrame:
    """Aggregate 30m Binance USD-M ETHUSDT klines to 8h OHLCV + taker_buy_share.

    The taker_buy_share column is computed as `taker_buy_quote / quote_volume`
    aggregated on the 8h bar boundary — it is the proxy for put-call ratio
    documented in data/manifest.txt.
    """
    raw = pd.read_parquet(parquet_path)
    raw["open_time"] = pd.to_datetime(raw["open_time"], unit="ms")
    raw = raw.set_index("open_time").sort_index()

    agg = raw.resample("8h").agg({
        "open": "first",
        "high": "max",
        "low": "min",
        "close": "last",
    ...
    # taker_buy_share = buy-aggressive volume fraction (PCR proxy).
    agg["taker_buy_share"] = (agg["taker_buy_quote"] / agg["quote_volume"]).clip(0.0, 1.0)

    df = agg[["open", "high", "low", "close", "volume", "taker_buy_share"]].copy()
    df.index.name = "ts"
    return df


def _ann_factor_for_tf(tf: str) -> float:
    """Annual bars-per-year for a 24/7 crypto timeframe."""
...
def _write_summary(result: dict, metrics: dict, tf: str, sym: str, starting_capital: float) -> dict:
    ...
    summary = {
        ...
        "data_source": "perp_30m_aggregated_to_8h_with_pcr_proxy_from_taker_buy_share",
        "starting_capital_usd": starting_capital,
        "per_symbol": [
            {
                "symbol": sym,
        ...
        "walk_forward": {"note": "config.json holds fold definitions; fold-by-fold evaluation requires running walk_forward.py separately."},
    }
    return summary
...
def _write_trades_csv(result: dict, tf: str, sym: str, csv_path: Path) -> None:
    ...
    for t in trades:
        rows.append({
            ...
            "pnl_usd": round(float(t["pnl_pct"]) * 10000.0, 4),  # proxy: pct * $100k
            "pnl_pct": round(float(t["pnl_pct"]), 6),
            "bars_held": int(t["bars_held"]),
            "pcr_z_at_entry": round(float(t.get("pcr_z_at_entry", 0.0)), 4),
...
def _write_equity_csv(result: dict, df_index: pd.DatetimeIndex, csv_path: Path) -> None:
    ...
    if len(df_index) >= n:
        ...
        extra_idx = pd.date_range(start=df_index[-1], periods=n_extra + 1, freq=inferred or "8h")[1:]
...
def main() -> None:
    ...
    df = _load_8h_ohlcv_with_pcr_proxy()
    ...
    print(json.dumps({
        "metrics": metrics,
        "summary": summary,
        "files": [str(trades_csv.name), str(equity_csv.name)],
