"""Run V1_funding_term_curve (vpvr_funding_term_curve_1h_20260714, iter#97) backtest.

Per-symbol sizing + per-symbol trades CSVs. Combined PnL book at book-level.
"""
...
def _write_trades_csv(trades, p: Path) -> None:
    fields = [
        ...
        "funding_z_at_entry", "poc_dist_atr_at_entry", "funding_paid_pct",
    ]
    with p.open("w", newline="") as fh:
        ...
        for t in trades:
            w.writerow({
                ...
                "poc_dist_atr_at_entry": f"{t.poc_dist_atr_at_entry:.3f}",
                "funding_paid_pct": f"{t.funding_paid_pct:.6f}",
            })
...
def main() -> int:
    ...
    agg_mdd = float(min((row["max_drawdown_pct"] for row in summary_rows), default=0.0))
    ...
    for row in summary_rows:
        print(
            f"V1_funding_term ({VARIANT_KEY}) {row['symbol']} iter#{cfg['iteration']} -> "
            f"sharpe={row['sharpe']:.3f} mdd={row['max_drawdown_pct']:.4f} "
            f"n={row['n_trades']} tag={row['tag']}"
        )
    print(
        f"V1_funding_term AGG -> sharpe={agg_sharpe:.3f} return={agg_return:.4f} "
