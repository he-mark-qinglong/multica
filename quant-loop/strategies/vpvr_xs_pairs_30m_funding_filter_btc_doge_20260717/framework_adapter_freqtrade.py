"""Freqtrade framework adapter for vpvr_xs_pairs_30m_funding_filter_btc_doge_20260717.

Cross-validate the in-house BTCUSDT/DOGEUSDT 30m xs-pair z-score +
VPVR confluence + funding-blowoff filter pair strategy (iter#86, NOT-PROFITABLE).

Replay notes
------------
- In-house pair-zscore convention mirrors the xs_basis family:
  pair direction is `long_a_short_b` (long BTC, short DOGE, pos=+1)
  or `short_a_long_b` (short BTC, long DOGE, pos=-1). Trades CSV
...
# 30m native BTCUSDT/DOGEUSDT parquets (per data/ listing):
...
try:
    ...
    class VPVRXsPairs30mDOGEFreqtradeStrategy(IStrategy):
        """Freqtrade IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_btc_doge_20260717."""
    ...
    class VPVRXsPairs30mDOGEFreqtradeStrategy(IStrategy):  # type: ignore[no-redef]
        def __init__(self, config: dict) -> None:
            self.config = config
            self.position = {"direction": "flat", "entry_ts": None,
                             "entry_a": 0.0, "entry_b": 0.0, "bars_held": 0}
