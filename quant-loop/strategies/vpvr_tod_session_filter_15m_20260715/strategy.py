"""vpvr_tod_session_filter_15m_20260715 — strategy backtest engine.
...
VARIANT_KEY = "vpvr_tod_session_filter_15m_20260715"
...
def run_backtest(df: pd.DataFrame, cfg: dict) -> dict:
    """Run single-symbol 15m backtest on a DataFrame with OHLCV columns."""
    p = cfg["params"]
    ...
    sess_arr = sig_df["session"].to_numpy()
    poc_dist_arr = sig_df["poc_distance_atr"].to_numpy()
    ...
    for i in range(1, len(df)):
        ...
        if pos == 0:
            ...
            if (
                ...
                entry_idx = i
                entry_px = px
                entry_session = str(sess_arr[i])
                entry_poc_dist = (
                    float(poc_dist_arr[i]) if np.isfinite(poc_dist_arr[i]) else 0.0
                )
                bars_held = 0
        else:
            bars_held += 1
            ...
            if exit_now:
                ...
                trades.append(Trade(
                    ...
                    session_at_entry=entry_session,
