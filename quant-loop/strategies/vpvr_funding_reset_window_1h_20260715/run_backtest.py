"""Run V3_funding_reset_window (iter#108) backtest.

TF = 1h. BTCUSDT. Funding rate from funding_analysis/.
Asymmetric 2:1 TP/SL execution.
"""
...
def _summarise(trades, equity, starting_cap, n_bars, timeframe):
    n = len(trades)
    base = {
    ...
    if n == 0:
        return {**base, "tag": "NOT-PROFITABLE",
                "data_status": "FUNDING-DATA-PRESENT funding_cadence=8h"}
    ...
    tp_n = sum(1 for t in trades if t.exit_reason == "tp_atr")
    sl_n = sum(1 for t in trades if t.exit_reason == "sl_atr")
    return {**base,
        ...
        "data_status": "FUNDING-DATA-PRESENT funding_cadence=8h",
        "tp_count": tp_n, "sl_count": sl_n,
        "tp_to_sl_ratio": tp_n / sl_n if sl_n > 0 else float("inf")}


def _write_trades_csv(trades, p):
    fields = ["variant", "symbol", "direction", "entry_ts", "entry_price",
              "exit_ts", "exit_price", "pnl_pct", "bars_held", "exit_reason",
              "poc_dist_at_entry", "funding_z_at_entry"]
    with p.open("w", newline="") as fh:
        ...
        for t in trades:
            w.writerow({
                ...
                "poc_dist_at_entry": f"{t.poc_dist_at_entry:.3f}",
                "funding_z_at_entry": f"{t.funding_z_at_entry:.3f}",
            })


def _write_equity_csv(equity, p):
    with p.open("w", newline="") as fh:
        w = csv.writer(fh)
...
def main():
    ...
    for sym, df in data.items():
        ...
        _write_trades_csv(res["trades"], RESULTS_DIR / f"trades_A_1h_{sym}.csv")
        _write_equity_csv(res["equity"], RESULTS_DIR / f"equity_1h_{sym}.csv")
    ...
    agg = {
        ...
        "by_symbol": {r["symbol"]: {
            ...
            "tp_to_sl_ratio": r.get("tp_to_sl_ratio", 0),
    ...
    (RESULTS_DIR / "summary.json").write_text(json.dumps({
        ...
        "timeframe": cfg["timeframe"], "instruments": cfg["instruments"],
        "axis": cfg["axis"], "tag": tag,
        "sharpe": agg_sharpe, "mdd": agg_mdd, "n_trades": agg_n,
        "profit_factor": agg_pf, "total_return_pct": agg_return,
        "per_symbol": summary_rows,
    }, indent=2, default=str))

    for r in summary_rows:
        print(
            f"V3_funding_reset ({VARIANT_KEY}) {r['symbol']} iter#{cfg['iteration']} -> "
            f"sharpe={r['sharpe']:.3f} mdd={r['max_drawdown_pct']:.4f} "
            f"n={r['n_trades']} tag={r['tag']} tp/sl={r.get('tp_count', 0)}/{r.get('sl_count', 0)}"
        )
    print(
        f"V3_funding_reset AGG -> sharpe={agg_sharpe:.3f} return={agg_return:.4f} "
