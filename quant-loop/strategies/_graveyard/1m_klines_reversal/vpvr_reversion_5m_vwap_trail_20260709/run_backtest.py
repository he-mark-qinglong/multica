"""Run V1: vpvr_reversion_5m_vwap_trail_20260709 (iter 65) — per symbol, with buy-hold baseline."""
...
def main() -> int:
    ...
    for sym, df in data.items():
        ...
        # Required by display engine: trades_<SYMBOL>__<timeframe>.csv
        trades_path = RESULTS_DIR / f"trades_A_{tf}_{sym}.csv"
        pd.DataFrame(_trade_rows(strat)).to_csv(trades_path, index=False)
        # Also write the unqualified form for symmetry with older strategies.
    ...
    # Aggregate across symbols (simple mean of per-symbol sharpes).
    ...
    print(f"\n=== V1 vpvr_reversion_5m_vwap_trail_20260709 iter={cfg['iteration']} ===")
