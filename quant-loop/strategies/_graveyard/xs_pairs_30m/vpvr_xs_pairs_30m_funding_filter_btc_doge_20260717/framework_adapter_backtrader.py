"""Backtrader framework adapter for vpvr_xs_pairs_30m_funding_filter_btc_doge_20260717.

Cross-validate the in-house BTCUSDT/DOGEUSDT 30m xs-pair z-score +
VPVR confluence + funding-blowoff-filter pair strategy (iter#86,
NOT-PROFITABLE).

Approach
--------
Replay the in-house trade log through backtrader with a synthetic
pair-spread encoding (BTCUSDT minus DOGEUSDT * normalization) so the
single-instrument backtrader broker tracks the paired pnl. Two data
feeds (BTCUSDT 30m and DOGEUSDT 30m) drive the entry/exit timing;
the synthetic spread encodes the pnl direction. The in-house
`pnl_pct` per trade is consumed by the strategy class to apply the
per-trade result on the exit bar.

The in-house convention mirrored here:
  - Per-bar GROSS mark on close-to-close: pnl_pct_per_bar = pos * (a_ret - b_ret) / 2.0
    pos=+1 for `long_a_short_b` (long BTC, short DOGE), pos=-1 for
    `short_a_long_b` (short BTC, long DOGE).
  ...
  - Backtrader cost = 24bp pair round-trip (3bp fee + 3bp slip per fill per leg × 4 fills).
  - Per-trade cost delta = +16bp = +0.0016 in pnl_pct space.
...
NOT-PROFITABLE strategy (campaign gate skipped walk-forward), so
...
STRATEGY = STRATEGY_DIR.name  # vpvr_xs_pairs_30m_funding_filter_btc_doge_20260717
DATA_DIR = STRATEGY_DIR / "data"
RESULTS_DIR = STRATEGY_DIR / "results"
TRADES_PATH = RESULTS_DIR / "trades_A_iter83_BTCUSDT_DOGEUSDT.csv"
EQUITY_PATH = RESULTS_DIR / "equity_A_iter83_BTCUSDT_DOGEUSDT.csv"
...
PRICE_PATH_DOGE_30M = DATA_DIR / "DOGEUSDT__30m.parquet"

W5_THRESHOLD = 50.0
TIMEFRAME = "30m"
SYMBOL_A, SYMBOL_B = "BTCUSDT", "DOGEUSDT"
ITERATION = 86
...
class VPVRXsPairs30mDOGEBacktraderStrategy(bt.Strategy):
    """Backtrader IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_btc_doge_20260717.
...
def replay_backtrader_bar_mtm(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    cost schedule (calibrated to in-house 8bp).
...
def _run_backtrader_engine(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    (BTCUSDT - DOGEUSDT * ratio_at_t0), normalized so 1 unit ≈ 1%
    ...
    cerebro.addstrategy(
        VPVRXsPairs30mDOGEBacktraderStrategy,
...
def main() -> int:
    ...
    # ---- Load and align price data (BTC 30m native, DOGE 30m native)
    btc = _load_30m_from_native(PRICE_PATH_BTC_30M)
    doge = _load_30m_from_native(PRICE_PATH_DOGE_30M)
    common = btc.index.intersection(doge.index).sort_values()
    if len(common) < 100:
        raise SystemExit(f"insufficient overlapping bars: {len(common)}")
    btc = btc.loc[common]
    doge = doge.loc[common]
    prices = pd.DataFrame({
        "ts": common,
        "close_a": btc["close"].to_numpy(dtype=float),
        "close_b": doge["close"].to_numpy(dtype=float),
    ...
    cv_record = {
        ...
        "data_source": {
            ...
            "doge_30m_path": str(PRICE_PATH_DOGE_30M),
        ...
        "inhouse_oos_walkforward": {
            ...
            "note": (
                ...
                "strategy (campaign gate skipped walk-forward)"
        ...
        "notes": [
            "Pair strategy (BTCUSDT/DOGEUSDT 30m): bar-by-bar MTM with `pos * (a_ret - b_ret) / 2.0`.",
            ...
            "iter#86 NOT-PROFITABLE BTC/DOGE pair variant.",
        ],
    }
