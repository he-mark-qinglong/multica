...
- **Task contracts (P0+P1, smark-approved 2026-07-26)** — full scheme: `docs/plans/contract-collab/03-contract-collaboration-scheme.md`. Rules: (1) **无契约不派单** — every dispatched issue body MUST open with a ```contract yaml block (template: `docs/templates/task-contract-template.yaml`); orchestrator generates it, workers read issue body as-is. (2) **CLAIMED_DONE ≠ DONE** — an agent's self-report only reaches CLAIMED_DONE; `VERIFIED_DONE` requires attestation via `quant-loop/scripts/contract_check.py --issue <id>` (exit 0). (3) **SIGNOFF comments MUST cite the attestation_run_id** (`attestation=att-xxxx`) — a PASS without one is void. (4) **Breach ladder** per task_id: 1st no-push/breach → redispatch to a DIFFERENT assignee; 2nd → orchestrator reviews the card itself; 3rd → freeze + escalate human. Env-fail checked before blaming the agent. (5) Contract fields worth defaulting: `valid_until` +24h, `timeout` 4h, `data_as_of` pins the latest usable data timestamp (anti-lookahead), mutable refs must pin SHA.
...
- **SPEC diagram rule (smark 2026-07-26)** — every SPEC/strategy doc MUST include ≥2 PNGs: structure diagram (entry/TP/SL on price path) + cost waterfall (gross margin vs locked cost basis). No diagrams = returned at SIGNOFF. Template: `quant-loop/_shared/templates/SPEC_TEMPLATE.md`.
...
- **Cost basis (updated 2026-07-26 20:45: 60% rebate confirmed real)** — nominal Binance VIP0 maker 2bp / taker 5bp per side × 40% = **effective maker 0.8bp / taker 2bp per side; pair-RT = maker 1.6bps / taker 4bps**. Rebate comes via a third-party aggregated-rebate account (smark-experienced) — its durability is a recorded risk factor in every verdict. se_h3 at rebate rates: maker 1.6bps → Sharpe **+8.88**, taker 4bps → **+5.98** (both live; KILL premise void — review at SMA-36660). Without rebate (taker 10bps) it was −1.40. Historical note: pre-rebate basis (SMA-36603) had made taker execution of <18bps-margin strategies structurally dead; that constraint is lifted at rebate rates but margin >4bps/trade remains the floor.
