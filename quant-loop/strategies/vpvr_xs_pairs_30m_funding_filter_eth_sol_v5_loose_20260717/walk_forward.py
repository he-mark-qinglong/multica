"""B6 walk-forward validation for V5 (vpvr_xs_pairs_30m_funding_filter_20260712).

Splits the available 30m data into rolling train/test windows, runs V5 on each
window, and reports per-window + aggregate metrics. Goal: verify that V5's
in-sample profitability holds up out-of-sample.

Why this matters for ship gate
------------------------------
V5's primary metrics.json reflects a single full-history backtest. Without a
walk-forward split we cannot tell whether the in-sample sharpe is structural
(pair-stat-arb alpha with funding filter) or fitted to the 2022-01 → 2026-07
sample. The walk-forward ratio (mean_oos_sharpe / mean_is_sharpe) is the
standard decay metric: >=0.5 = shippable, 0-0.5 = degraded, <0 = overfit.

Windows (anchored expanding-train)
----------------------------------
Train init = 9 months, test = 6 months, step = 3 months. Test windows span
2023-07-01 → 2026-07-01 across ~3 windows for the 30m BTC/SOL span.

Note: V5's strategy.py re-fits z-score and VPVR bars on every bar (rolling
...
Usage
-----
    PYTHONPATH=. python3 walk_forward.py [--n-windows 3] [--test-months 6]
"""
...
def main(argv=None) -> int:
    ...
    for w in windows:
        ...
        result = run_backtest(data, cfg, funding=funding)
