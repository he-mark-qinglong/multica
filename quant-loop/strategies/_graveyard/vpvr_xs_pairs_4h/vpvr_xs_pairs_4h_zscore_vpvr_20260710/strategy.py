...
def _rolling_vpvr(close: np.ndarray, volume: np.ndarray, window: int, n_bins: int) -> dict:
    """Rolling VPVR POC + HVN density computed strictly from past bars (no look-ahead).

    Returns a dict with:
      - ``poc``: per-bar POC (midpoint of the highest-volume bin in the lookback window)
      - ``near_hvn``: per-bar boolean — True iff current close's bin volume exceeds
        ``hvn_threshold`` fraction of the max bin volume in the lookback window
    """
    n = len(close)
    ...
    for i in range(window, n):
        ...
        max_v = float(np.max(bin_v))
        if max_v <= 0:
            continue
        poc[i] = float((bins[int(np.argmax(bin_v))] + bins[int(np.argmax(bin_v)) + 1]) / 2.0)
    return {"poc": poc, "near_hvn": near_hvn}


def _rolling_vpvr_with_hvn(
    close: np.ndarray, volume: np.ndarray, window: int, n_bins: int, hvn_threshold: float
) -> dict:
    """Same as ``_rolling_vpvr`` but also computes ``near_hvn`` boolean per bar.

    ``near_hvn[i]`` is True when the bin that contains ``close[i]`` has cumulative
    volume >= ``hvn_threshold`` * max_bin_volume over the same lookback window.
    A higher ``hvn_threshold`` makes the filter STRICTER (only very high-volume
    bins qualify); a lower value is more permissive.
    """
    n = len(close)
    poc = np.full(n, np.nan)
    near_hvn = np.zeros(n, dtype=bool)
    for i in range(window, n):
        ...
        if max_v <= 0:
            continue
        poc[i] = float((bins[int(np.argmax(bin_v))] + bins[int(np.argmax(bin_v)) + 1]) / 2.0)
        cur_idx = int(np.clip(np.digitize(close[i], bins) - 1, 0, n_bins - 1))
        if bin_v[cur_idx] >= hvn_threshold * max_v:
            near_hvn[i] = True
    return {"poc": poc, "near_hvn": near_hvn}
...
def _round_trip_cost_pct(cfg: dict) -> float:
    """Return round-trip cost as a fraction of notional.

    Prefers ``cfg["cost_bps_total"]`` when supplied (per-task override),
    otherwise falls back to ``fees_bps_per_side + slippage_bps_per_side``
    multiplied by 2 legs × 2 sides (entry+exit on each leg).
    """
    if "cost_bps_total" in cfg:
        return float(cfg["cost_bps_total"]) / 10_000.0
    fee = float(cfg.get("fees_bps_per_side", 0.0))
    slip = float(cfg.get("slippage_bps_per_side", 0.0))
...
def run_pair_backtest(df_a: pd.DataFrame, df_b: pd.DataFrame, cfg: dict, pair_label: str) -> dict:
    ...
    # New VPVR knobs:
    #   vpvr_poc_attractor_strength ∈ [0.3, 1.0]: scales the proximity threshold.
    #     1.0 = baseline, 0.5 = half-width (tighter), 0.3 = 30% width (very tight).
    #   vpvr_hvn_threshold ∈ [0.5, 0.9]: a bin must have this fraction of the
    #     window-max volume to qualify as a "high-volume node". 0 = filter off.
    attractor = float(ind.get("vpvr_poc_attractor_strength", 1.0))
    hvn_thr = float(ind.get("vpvr_hvn_threshold", 0.0))
    if hvn_thr > 0:
        profile = _rolling_vpvr_with_hvn(
            a["close"].to_numpy(), a["volume"].to_numpy(),
            int(ind["vpvr_window_bars"]), int(ind["vpvr_n_bins"]), hvn_thr
        )
    else:
        profile = _rolling_vpvr(
            a["close"].to_numpy(), a["volume"].to_numpy(),
            int(ind["vpvr_window_bars"]), int(ind["vpvr_n_bins"])
        )
    poc = pd.Series(profile["poc"], index=a.index)
    near_hvn = pd.Series(profile["near_hvn"], index=a.index)
    proximity = float(ind["vpvr_proximity_atr_k"]) * attractor * a_atr
    ...
    require_confluence = bool(cfg["entry"].get("require_vpvr_confluence", True))
    require_hvn = bool(cfg["entry"].get("require_hvn", hvn_thr > 0))
    for i in range(1, n):
        cur_pos = pos
        zi = float(z.iat[i]) if np.isfinite(z.iat[i]) else None
        if cur_pos == 0 and zi is not None and require_confluence and not bool(near_poc.iat[i]):
            cur_pos = 0
        if cur_pos == 0 and zi is not None and require_hvn and not bool(near_hvn.iat[i]):
            cur_pos = 0
        ...
        if cur_pos != 0 and pos == 0:
            ...
            elif require_hvn and not bool(near_hvn.iat[i]):
                exit_reason = "confluence_lost"
            if abs(zi) <= exit_thr:
