"""Run V7 (xs-pair z-score + VPVR confluence + funding-blowoff filter, regularized iter#83)."""
...
def main():
    ...
    for pr in per_pair_results:
        ...
        _write_trades(pr["trades"], RESULTS_DIR / ("trades_" + VARIANT_KEY + "_iter83_" + label + ".csv"))
        equity_idx = pd.date_range(pr["span_start"], periods=len(pr["equity"]), freq="30min")
        _write_equity(pr["equity"], equity_idx,
                      RESULTS_DIR / ("equity_" + VARIANT_KEY + "_iter83_" + label + ".csv"))
