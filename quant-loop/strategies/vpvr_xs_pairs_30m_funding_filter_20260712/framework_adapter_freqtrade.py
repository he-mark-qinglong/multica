"""Freqtrade framework adapter for vpvr_xs_pairs_30m_funding_filter_20260712.

Cross-validate the in-house BTCUSDT/SOLUSDT 30m xs-pair z-score + VPVR
confluence + funding-blowoff-filter pair strategy (iter#81, PROFITABLE).
...
- Freqtrade replay computes a fresh gross pnl per trade from entry/exit
  ...
  equity curve at the exit bar absorbs `risk_target * (gross - cost_rt)`
  in addition to the bar mark, mirroring freqtrade's IStrategy contract
  for a pair strategy.
- Walk-forward OOS = 3 contiguous chronological folds over the 30m
  replay window (mirrors the xs-basis pairing pattern).
...
try:
    ...
    class VPVRXsPairs30mFundingFilterFreqtradeStrategy(IStrategy):
        """Freqtrade IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_20260712."""
    ...
    class VPVRXsPairs30mFundingFilterFreqtradeStrategy(IStrategy):  # type: ignore[no-redef]
        def __init__(self, config: dict) -> None:
            self.config = config
            self.position = {"direction": "flat", "entry_ts": None,
                             "entry_a": 0.0, "entry_b": 0.0, "bars_held": 0}
...
def replay_inhouse_bar_mtm(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    # In-house convention (strategy.py:246-252): the entry bar itself does
    # NOT get a bar mark (pnl_pct_per_bar[entry_idx] is not set); the mark
    # applies from entry_idx+1 through the exit bar inclusive (exit bar is
