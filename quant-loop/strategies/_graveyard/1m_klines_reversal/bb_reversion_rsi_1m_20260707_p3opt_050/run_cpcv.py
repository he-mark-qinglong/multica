"""CPCV for P3-OPT-050 with apples-to-apples baseline comparison.

Variant: entry_threshold (indicators.bb_k) = 2.5  — strategy loaded from config.json.
...
and the same code path. The single-variable change is isolated to the entry
parameter (Bollinger Band k multiplier).
...
def main() -> int:
    ...
    print(
        f"[{datetime.now(timezone.utc).isoformat()}] P3-OPT-050 CPCV start "
    ...
    summary_lines = [
        f"{cfg['strategy']} — P3-OPT-050 single-variable sweep",
        f"entry_threshold variant=bb_k={cfg['indicators']['bb_k']} baseline=bb_k=2.0",
