"""Data loader for the U5 funding-oscillator MR strategy (SMA-34957).

Reuses the exact same data sources as the U5 funding-carry harness:
real Binance USDT-M perp 1m OHLCV + 8h funding events.

  ETH 1m OHLCV     -> data/perp_1m/ETHUSDT_1m.parquet
  SOL 1m OHLCV     -> strategies/vpvr_volume_edge_3tf_v1_20260711/
                      data/SOLUSDT__1m.parquet (real Binance snapshot;
                      verified provenance via SHA256 match with the
                      vpvr_xs_pairs mirror copy).
  ETH funding      -> data/funding/ETHUSDT.parquet
  SOL funding      -> data/funding/SOLUSDT.parquet
"""
...
def load_symbol_1m(symbol: str, window_days: int) -> Tuple[pd.DataFrame, dict]:
    ...
    (df, funding_event_stats)
        ...
        ``open, high, low, close, volume, funding``. ``funding_event_stats``
        describes the raw event distribution over the loaded window.
    """
    sym = symbol.upper()
    if sym == "ETHUSDT":
    ...
    stats = {
        ...
        "ge_1bp_pct": float((events >= 0.0001).mean()) if len(events) else 0.0,
