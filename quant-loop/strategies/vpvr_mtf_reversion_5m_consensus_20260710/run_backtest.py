"""Run V7: vpvr_mtf_reversion_5m_consensus_20260710 (iter 71) — per symbol, with buy-hold baseline."""
    ...
    return [
        {
            ...
            "htf_trend_at_entry": t.htf_trend_at_entry,
        }
...
def main() -> int:
    ...
    data = load_all(cfg["instruments"], tf, htf)
    summary_rows = []
    ...
    for sym, frames in data.items():
        ...
        strat = run_backtest(df_ltf, df_htf, cfg_t)
        hold = baseline_hold(df_ltf, cfg_t)

        strat.equity_curve.to_frame("equity").to_csv(RESULTS_DIR / f"equity_{sym}.csv")
        trades_path = RESULTS_DIR / f"trades_A_{tf}_{sym}.csv"
        pd.DataFrame(_trade_rows(strat)).to_csv(trades_path, index=False)
        pd.DataFrame(_trade_rows(strat)).to_csv(RESULTS_DIR / f"trades_{sym}.csv", index=False)

        summary_rows.append({
            "symbol": sym,
            "rows": len(df_ltf),
            "span_start": df_ltf.index[0].date().isoformat(),
            "span_end": df_ltf.index[-1].date().isoformat(),
            "timeframe": tf,
    ...
    # Emit metrics.json — Evidence Gate schema: sharpe / mdd / n_trades.
    agg = {
        "iteration": cfg["iteration"],
        "axis": cfg.get("axis"),
        "timeframe": tf,
    ...
    print(f"\n=== V7 vpvr_mtf_reversion_5m_consensus_20260710 iter={cfg['iteration']} axis=mtf_consensus ===")
