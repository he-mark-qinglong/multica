"""Signal builder for vpvr_sentiment_attention_1m_20260716.
...
Input columns:
    ...
    - attention: raw social-attention / social-volume metric
...
Output columns:
    ...
    - attention_z: z-score of attention
...
def build_signals(df: pd.DataFrame, params: dict) -> pd.DataFrame:
    """Build entry/exit signals for the sentiment-attention VPVR reversion variant.

    Args:
        df: DataFrame with columns [open, high, low, close, volume, attention].
    ...
    attention = df["attention"].astype(np.float64)

    poc = _vpvr_poc(close, volume, params["vpvr_window_bars"], params["vpvr_bins"])
    atr = _atr(df, params["atr_period"])
    attention_z = _z_score(attention, params["attention_z_lookback_bars"])

    atr_safe = atr.replace(0, np.nan)
    poc_distance_atr = (close - poc).abs() / atr_safe

    z_thr = params["attention_z_threshold"]
    poc_thr = params["poc_atr_buffer"]

    long_signal = (close <= poc) & (poc_distance_atr <= poc_thr) & (attention_z >= z_thr)
    short_signal = (close >= poc) & (poc_distance_atr <= poc_thr) & (attention_z >= z_thr)
