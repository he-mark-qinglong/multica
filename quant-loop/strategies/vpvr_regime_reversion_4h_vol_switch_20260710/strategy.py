"""VPVR-aware 4h strategy: vol-regime-switch reversion.

This is a **fresh** implementation — it does not import or extend any
existing ``vpvr_*.py``. The axis is volatility regime detection: we
classify each bar as either 'low' or 'high' realized-volatility regime,
look for the **low → high transition**, and only then enter a mean-reversion
trade back toward the VPVR POC.

The hypothesis: regime switches mark the start of a vol-expansion episode.
Within that episode, a re-test of POC is a high-quality entry because the
volume profile is still anchored there. Once the regime relaxes (rv drops
back to low), the entry has run its course and we exit.

Indicators (all computed from data in ``[t-W, t-1]`` only — no look-ahead):

  * ``rv_bps``        — annualized realized volatility in basis points.
  * ``rv_regime``     — boolean: True if rv_bps >= threshold (high regime).
  * ``regime_switch`` — boolean: True iff regime changed from low to high within the lookback window.
  * ``vpvr``          — 4h rolling volume profile (price bins × total volume).
...
Entry (long):  regime_switch[t] AND price just touched VAL (within 1 ATR) of POC.
Entry (short): regime_switch[t] AND price just touched VAH (within 1 ATR) of POC.

Exit (first triggered wins):
    1. Regime reversal: regime flips back to low.
    2. Stop loss: 4 * ATR against entry.
...
def realized_vol_bps(df: pd.DataFrame, window: int, periods_per_year: float) -> pd.Series:
    """Annualized realized volatility in basis points.

    Computed as: stdev(log_returns, window) * sqrt(periods_per_year) * 10000.
    """
    log_ret = np.log(df["close"] / df["close"].shift(1))
    rv = log_ret.rolling(window, min_periods=window).std() * math.sqrt(periods_per_year) * 10000.0
    return rv


def regime_switch_signal(rv: pd.Series, threshold_bps: float, lookback_bars: int) -> pd.Series:
    """Boolean: True if regime transitioned from low to high within the lookback window.

    Specifically, for bar ``t`` we check whether there is at least one bar
    in ``[t - lookback_bars + 1, t]`` whose ``rv < threshold`` followed by a
    bar with ``rv >= threshold``. The flag is True on the first high bar after
    such a transition.
    """
    is_high = (rv >= threshold_bps).fillna(False).to_numpy()
    n = len(is_high)
    ...
    for i in range(n):
        if not is_high[i]:
            last_low_idx = i
            continue
        # We are in high regime here. Did we see a low bar within lookback?
        if (i - last_low_idx) <= lookback_bars and last_low_idx >= 0:
            flag[i] = True
    return pd.Series(flag, index=rv.index, name="regime_switch")
...
def poc_test_signal(df: pd.DataFrame, profile: pd.DataFrame, atr: pd.Series, atr_k: float) -> pd.DataFrame:
    """Detect touches of VPVR VAL or VAH within ``atr_k * ATR`` of POC.

    For each bar:
        * ``poc_low_test`` — close within atr_k*ATR of vpvr_val (downside POC test).
        * ``poc_high_test`` — close within atr_k*ATR of vpvr_vah (upside POC test).

    Returns a frame with these two boolean columns.
    """
    val = profile["vpvr_val"]
    vah = profile["vpvr_vah"]
    band = (atr_k * atr).fillna(0.0)

    low_test = ((df["close"] - val).abs() <= band).fillna(False)
    high_test = ((df["close"] - vah).abs() <= band).fillna(False)
    return pd.DataFrame({"poc_low_test": low_test, "poc_high_test": high_test}, index=df.index)
...
def annotate(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    # 4h bars per year: 6 per day * 365 = 2190.
    out["rv_bps"] = realized_vol_bps(out, rg["rv_window_bars"], periods_per_year=2190.0)
    out["rv_regime"] = (out["rv_bps"] >= rg["rv_threshold_bps"]).fillna(False)
    out["regime_switch"] = regime_switch_signal(out["rv_bps"], rg["rv_threshold_bps"], rg["lookback_bars"])

    # recent_switch[t] = True if any regime_switch occurred in [t - switch_recent_bars + 1, t].
    # This widens the entry window: a POC re-test within a few bars of the switch counts.
    recent = rg.get("switch_recent_bars", 1)
    if recent <= 1:
        out["recent_switch"] = out["regime_switch"].fillna(False)
    else:
        out["recent_switch"] = (
            out["regime_switch"]
            .fillna(False)
            .rolling(window=recent, min_periods=1)
            .max()
            .fillna(False)
            .astype(bool)
        )
    ...
    poc_tests = poc_test_signal(out, profile, out["atr"], vp["poc_touch_atr"])
    out = pd.concat([out, poc_tests], axis=1)

    out["long_entry"] = (
        out["recent_switch"]
        & out["poc_low_test"].fillna(False)
        & out["atr"].notna()
        & out["vpvr_poc"].notna()
    )
    out["short_entry"] = (
        out["recent_switch"]
        & out["poc_high_test"].fillna(False)
        & out["atr"].notna()
        & out["vpvr_poc"].notna()
    )
...
def _exit_on_bar(bar: pd.Series, direction: str, entry_price: float, atr_at_entry: float, cfg: dict) -> Tuple[bool, str, float]:
    ...
    # 1. Regime reversal: high -> low.
    if bool(bar.get("rv_regime", True)) is False:
        return True, "regime_reversal_to_low", cur

    # 2. Stop loss.
    if direction == "long":
        if cur <= entry_price - ex["stop_loss_atr_k"] * atr_now:
            return True, f"stop_loss<-{ex['stop_loss_atr_k']}*ATR", cur
    else:
        if cur >= entry_price + ex["stop_loss_atr_k"] * atr_now:
            return True, f"stop_loss>+{ex['stop_loss_atr_k']}*ATR", cur
    ...
    for i, (date, row) in enumerate(df.iterrows()):
        ...
        if in_pos is None:
            if bool(row.get("long_entry", False)) and (open_notional_pct + per_signal) <= max_gross + 1e-12:
                ...
                rv_at_entry = float(row["rv_bps"]) if not pd.isna(row.get("rv_bps", np.nan)) else 0.0
                entry_idx = i
                entry_date = date
            ...
            if exit_now:
                ...
                trades.append(
                    Trade(
                        ...
                        bars_held=bars_held, rv_bps_at_entry=rv_at_entry,
                    )
                )
                equity += pnl_abs
                equity_path[-1] = (date, equity)
    ...
    if in_pos is not None:
        ...
        trades.append(
            Trade(symbol=symbol, direction=in_pos, entry_date=entry_date,
                  ...
                  bars_held=len(df) - 1 - entry_idx, rv_bps_at_entry=rv_at_entry)
        )
        equity += pnl_abs
