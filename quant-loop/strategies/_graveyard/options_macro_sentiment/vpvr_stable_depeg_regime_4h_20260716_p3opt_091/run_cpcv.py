"""CPCV harness for vpvr_stable_depeg_regime_4h_20260716_p3opt_091.

P3-OPT-091 single-variable sweep. Uses the shared CPCV harness
(_shared.validation.cpcv) with n_groups=6, k_test=2 as required by the
issue. Synthetic data is generated by run_backtest._make_synthetic_data
(identical seed) so the variant vs baseline comparison is apples-to-apples.
...
PURGE_BARS = 50
EMBARGO_BARS = 20
PERIODS_PER_YEAR_4H = 2190   # 365 * 6


def _build_strategy_returns(df: pd.DataFrame, cfg: dict) -> pd.Series:
    """Run the strategy on the full synthetic data; return per-bar returns."""
...
def main() -> int:
    ...
    envelope = {
        ...
        "single_param_changed": "signal_blend_weight=0.5",
        "aggregate": aggregate,
        "verdict": verdict,
        "folds": folds_payload,
    }
    ...
    lines: List[str] = []
    lines.append(f"=== {VARIANT_KEY} CPCV (P3-OPT-091) ===")
    lines.append(f"  single_param_changed: signal_blend_weight=0.5")
    lines.append(
