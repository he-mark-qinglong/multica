"""Freqtrade framework adapter for vpvr_xs_pairs_30m_funding_filter_eth_sol_20260712.
...
NOT-PROFITABLE in-house). Both legs are resampled from 15m to 30m.
...
- Data handling mirrors `data_loader.py`: both ETHUSDT and SOLUSDT are
  loaded from native 15m parquets and resampled to 30m with
...
# Both legs are native 15m parquets and are resampled to 30m below.
...
try:
    ...
    class VPVRXsPairs30mETHSOLFreqtradeStrategy(IStrategy):
        """Freqtrade IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_eth_sol_20260712."""
    ...
    class VPVRXsPairs30mETHSOLFreqtradeStrategy(IStrategy):  # type: ignore[no-redef]
        def __init__(self, config: dict) -> None:
            self.config = config
            self.position = {"direction": "flat", "entry_ts": None,
                             "entry_a": 0.0, "entry_b": 0.0, "bars_held": 0}
...
def main() -> int:
    ...
    # ---- Load and align price data: ETH and SOL 15m resampled to 30m
    ...
    # ---- Framework (freqtrade) replay with INHOUSE_COST_RT_PAIR (calibrated 8bp) to match in-house
    ...
    # walk_forward.json: this NOT-PROFITABLE strategy produced one (iter#82)
    ...
    cv_record = {
        ...
        "inhouse_oos_walkforward": {
            ...
            "note": (
                "from walk_forward.json (iter#82 produced walk_forward.json)"
        ...
        "notes": [
            ...
            "iter#82 ETH/SOL pair variant, in-house tag=NOT-PROFITABLE (sharpe 0.179 / total_return 6.34% / max_dd -24.47%).",
