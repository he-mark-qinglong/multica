"""Run V9 (xs-pair z-score + VPVR confluence + funding-blowoff filter, regularized v3 iter#85)."""
...
def _summarise_pair(pair_result, cfg):
    ...
    dd = equity / peak - 1.0
    max_dd = float(dd.min())
    # 30m bars per day = 48 → span_days = n_bars * 30m / (60*24)
    span_days = max(pair_result["n_bars"] * 30.0 / (60.0 * 24.0), 1)
...
def _write_trades(trades, p):
    fields = ["variant", "pair", "direction", "entry_ts", "entry_price_a",
              ...
              "funding_ema_at_entry", "exit_reason"]
...
def main():
    ...
    funding = load_funding(instruments)
    ...
    for pr in per_pair_results:
        ...
        _write_trades(pr["trades"], RESULTS_DIR / ("trades_" + VARIANT_KEY + "_iter85_" + label + ".csv"))
        equity_idx = pd.date_range(pr["span_start"], periods=len(pr["equity"]), freq="30min")
        _write_equity(pr["equity"], equity_idx,
                      RESULTS_DIR / ("equity_" + VARIANT_KEY + "_iter85_" + label + ".csv"))
