"""Vectorbt framework adapter for vpvr_funding_reset_window_1h_20260715.
...
closes, changing ONLY the cost model to vectorbt's canonical zero-cost
...
Compared to the in-house 12 bp rt (4 bp fee + 2 bp slip, per config.json)
and freqtrade's 12 bp rt and backtrader's 14 bp rt, vectorbt's 0 bp rt is a
-12 bp cost delta per trade (vectorbt zero-cost vs in-house). That delta is
large enough to surface any cost-model divergence in the W5 metric check
while remaining a defensible vectorbt convention.

Validation step first: replay at in-house cost (12 bp rt) using the same
risk_per_trade-scaled bar MTM convention used by the backtrader/freqtrade
adapters for this strategy — must reproduce the in-house equity CSV to
within the same ~6e-6 tolerance band they hit.
...
VECTORBT_COST_RT = 2.0 * (VECTORBT_FEE_BPS_PER_SIDE
                          + VECTORBT_SLIP_BPS_PER_SIDE) / 1e4  # 0.0
...
def main() -> int:
    ...
    print(f"[validation BTCUSDT] bars={v['n_bars_compared']} max_rel_err={v['max_abs_rel_err']:.6e} "
          f"final_rel_err={v['final_rel_err']:.6e} replay_dd={v['replayed_max_dd']:.6e} "
          f"ih_dd={v['inhouse_max_dd']:.6e}")
    ...
    cost_delta = fw_cost_rt - inhouse_cost_rt  # 0bp - 12bp = -12bp = -0.0012 per trade
    ...
    # ---- 3) divergence vs metrics.json agg_* (same targets as backtrader/freqtrade runs)
    ...
    results = {
        ...
        "fix_note": ("mirrors the in-house risk_per_trade-scaled equity construction "
                     ...
                     "differs: vectorbt canonical zero-cost reference convention "
                     "(fee=0, slip=0 -> 0bp round trip), vs in-house 12bp rt "
                     "(4bp fee + 2bp slip), freqtrade 12bp rt, backtrader 14bp rt. "
                     "Validated first by replaying at in-house cost and matching "
                     "the equity CSV."),
        "cost_model": {"fee_bps_per_side": VECTORBT_FEE_BPS_PER_SIDE,
        ...
        "approach": ("vectorbt 1.1.0 zero-cost reference convention (fee=0, slip=0 "
                     "-> 0bp rt) applied to the in-house entry/exit schedule; "
                     "equity walk is risk_per_trade-scaled bar MTM (equity *= "
                     "(1 + scale * bar_ret)) on real 1h closes, cost amortised over "
                     "held bars; validated by reproducing the in-house equity CSV at "
