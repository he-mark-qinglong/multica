...
- **Infra sprint status (2026-07-25 17:15)** — Wave-0 CLOSED (24/24 merged, parent SMA-36455). Wave-1 dispatched: 24 tasks → multica-code ×13 (mac), strategy-worker-1 ×4 / strategy-worker-2 ×3 / ops-worker-1 ×4 (.105). Critical path: W4-T06 parity dual-anchor gate (==base engine AND ==quick_verify 704 trades/8.0735) gates Wave 2/3 W4 — orchestrator verifies personally. Assignments live in `docs/plans/infra-sprint-2026-07-25/issue-map.json`. All `framework_adapter_*.py` retired repo-wide (W1-T4, ae3adb60f). If `pnpm typecheck` goes red with broken `@multica/*` symlinks, run `pnpm install` first (fixed 2026-07-25, 6/6 green).
...
- **Strategy state (2026-07-25)** — NO live-eligible strategy exists (ledger rebuilt under fixed gate semantics: 0 PASS out of 99). mtf_xs_pairs variants re-validated on the unified baseline methodology after fixing a 2× cost double-count in the variants runner: **H3 (OOS Sharpe 1.875, CI lower 0.888) > H1 (1.597 / 0.832) > H4 (1.187 / 0.378) > H2 (0.369 / -0.543)**. H1 is the only fee-robust variant (60bps fee-shock Sharpe +0.728 vs H3 −0.021). Family gross edge ≈2.5bps/trade — razor-thin vs costs; honest per-bar cost debit kills all four ≥~2.5bps/trade. Best open lead: `signal-enhance-h3` (2024-subsample Sharpe 8.07, NOT yet validated on full history). Killed families (1m/5m klines reversal, funding-carry, 4h single-TF stat-arb, microstructure features) must not be parameter-swept again (cycle-46). 57 falsified strategy dirs archived in `quant-loop/strategies/_graveyard/`.
...
- **Daemon bandwidth (2026-07-25)** — `MULTICA_DAEMON_MAX_CONCURRENT_TASKS`: **Mac=6, .105=20**. Mac is the bottleneck: 16GB RAM fully used + load 13 at 6 slots — do NOT raise it (it's also the user's workstation). .105 has big headroom (23GB/16 cores, load <1 at 20 slots); raise further via `~/.config/systemd/user/multica-daemon.service` + `systemctl --user daemon-reload && restart multica-daemon` — restart is safe, interrupted tasks return to queue via claim-lease expiry. Per-agent caps: workers max_concurrent=20, quant-researcher=6, smark-*=3. Known issue: L1 autopilot dispatches starve behind L3 batches on the full Mac daemon — fix spec'd in SMA-36539 (wait_reason classification + fairness).
