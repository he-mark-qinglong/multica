...
(async () => {
  ...
  for (const [name, ratio, subplot] of [['top',0.18,'xy'],['middle',0.45,'x2y2'],['bottom',0.72,'x3y3']]) {
    const result = await page.evaluate(({clientX, clientY, subplot}) => {
      ...
      try { Plotly.Fx.hover(gd, [{curveNumber:cand, pointNumber:best}], subplot); } catch(e){ return {error:e.message}; }
    ...
    }, {clientX: box.x + box.width/2, clientY: box.y + box.height*ratio, subplot});
