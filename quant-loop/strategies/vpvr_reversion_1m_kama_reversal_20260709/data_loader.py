"""Data loader for vpvr_reversion_1m_kama_reversal_20260709 (iter 67).

Timeframe is 1m, so we copy the source 1m parquet directly instead of
resampling. SHA256 manifest is still computed against the canonical upstream.
"""
...
def load_symbol(
    ...
    if not src.exists():
        raise FileNotFoundError(f"missing source for {sym}: {src}")
    if timeframe == "1m":
        ...
        df.to_parquet(cache)
        return df
    # Fall back to resample for any other timeframe.
    pandas_rule = {"5m": "5min", "15m": "15min", "1h": "1h", "4h": "4h", "1d": "1D"}.get(timeframe, timeframe)
    df = _read_1m(src)
    agg = {"open": "first", "high": "max", "low": "min", "close": "last", "volume": "sum"}
    out = df.resample(pandas_rule, label="left", closed="left").agg(agg).dropna(subset=["close"])
