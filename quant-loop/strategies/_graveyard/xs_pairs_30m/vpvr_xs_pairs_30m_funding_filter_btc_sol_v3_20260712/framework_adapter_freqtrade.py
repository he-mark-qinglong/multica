"""Freqtrade framework adapter for vpvr_xs_pairs_30m_funding_filter_btc_sol_v3_20260712.

Cross-validate the in-house BTCUSDT/SOLUSDT 30m xs-pair z-score +
VPVR confluence + funding-blowoff filter pair strategy (iter#85, v3,
PROFITABLE in-house). v3 parameter set: entry_z=2.8, lookback=288,
max_hold=144, fund_thr=0.0002.
...
try:
    ...
    class VPVRXsPairs30mSOLV3FreqtradeStrategy(IStrategy):
        """Freqtrade IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_btc_sol_v3_20260712."""
    ...
    class VPVRXsPairs30mSOLV3FreqtradeStrategy(IStrategy):  # type: ignore[no-redef]
        def __init__(self, config: dict) -> None:
            self.config = config
            self.position = {"direction": "flat", "entry_ts": None,
                             "entry_a": 0.0, "entry_b": 0.0, "bars_held": 0}
...
def main() -> int:
    ...
    cv_record = {
        ...
        "notes": [
            ...
            "Validation: replay trades with cost_rt = INHOUSE_COST_RT_PAIR (matches in-house exit-bar cost debit).",
            ...
            "iter#85 v3 BTC/SOL pair variant, in-house tag=PROFITABLE (sharpe 2.30 / total_return 622.78% / max_dd -19.23%).",
