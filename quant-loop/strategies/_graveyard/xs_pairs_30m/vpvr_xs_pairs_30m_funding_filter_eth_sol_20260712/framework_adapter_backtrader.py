"""Backtrader framework adapter for vpvr_xs_pairs_30m_funding_filter_eth_sol_20260712.

Cross-validate the in-house ETHUSDT/SOLUSDT 30m xs-pair z-score +
VPVR confluence + funding-blowoff filter pair strategy (iter#82,
NOT-PROFITABLE per current metrics.json tag).
...
broker path is also exercised on a synthetic pair-spread series
(ETH - SOL * ratio_at_t0, normalised) to verify the calibrated
replay per `VPVRXsPairs30mETHSOLBacktraderStrategy`.
...
The in-house convention mirrored here:
  ...
  - Terminal open position is detected and held through the final
    bar (mirrors the freqtrade adapter's terminal_open_position handling).
  ...
  - In-house cost = 8bp pair round-trip per the strategy's config.json
    ...
    The smark-proxy 2026-07-20 DECISION normalised cost basis to 24bp for
    the BTC/SOL pair strategies (v3 etc.); the ETH/SOL pair's in-house
    equity was generated before that normalisation at the local 8bp basis
    so we use the 8bp basis here for validation fidelity.
...
Data handling mirrors `data_loader.py`:
  - ETHUSDT native 15m parquet → resample_ohlcv(rule="30min") to 30m
    (open=first, high=max, low=min, close=last, volume=sum).
  - SOLUSDT native 15m parquet → resample_ohlcv(rule="30min") to 30m.
  - Both legs share the same resample path (no native 30m parquet).
  - ETHUSDT funding + SOLUSDT funding parquets exist but the trade log
    carries the realised pnl after the funding-blowoff filter; they are
    not needed for the replay pipeline (the funding gate only changes
    WHICH trades enter, not the per-trade pnl shape).

Walk-forward OOS = 3 contiguous chronological folds over 2023-01 →
2024-07. In-house walk_forward.json IS produced for this iter#82
(NOT-PROFITABLE strategy still produced walk_forward.json in this
campaign). The mean_test_sharpe / mean_test_return / worst_test_mdd
from walk_forward.json aggregate are the OOS reference (per the
freqtrade adapter precedent on the same strategy).
...
STRATEGY = STRATEGY_DIR.name  # vpvr_xs_pairs_30m_funding_filter_eth_sol_20260712
DATA_DIR = STRATEGY_DIR / "data"
RESULTS_DIR = STRATEGY_DIR / "results"
TRADES_PATH = RESULTS_DIR / "trades_A_iter81_ETHUSDT_SOLUSDT.csv"
EQUITY_PATH = RESULTS_DIR / "equity_A_iter81_ETHUSDT_SOLUSDT.csv"
...
PRICE_PATH_ETH_15M = DATA_DIR / "ETHUSDT__15m.parquet"
...
# In-house cost (legacy config basis at the time the equity CSV was generated):
# 1bp fee + 1bp slip per side per leg × 2 legs × 2 sides = 8bp pair RT.
...
# Backtrader cost: calibrated 1:1 with in-house (8bp basis) for apples-to-apples
# comparison. The smark-proxy 2026-07-20 DECISION normalised cost basis to 24bp
# for the BTC/SOL pair strategies; the ETH/SOL pair's in-house equity CSV was
# generated before that normalisation so we use the 8bp basis here for validation
# fidelity. Per-trade cost delta = 0bp at the 8bp basis.
...
# Framework replay is calibrated to in-house cost (8bp legacy config basis)
...
if _HAS_BACKTRADER:
    class VPVRXsPairs30mETHSOLBacktraderStrategy(bt.Strategy):
        """Backtrader IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_eth_sol_20260712.
    ...
    class VPVRXsPairs30mETHSOLBacktraderStrategy:  # type: ignore[no-redef]
...
def replay_inhouse_bar_mtm(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    the in-house equity walk. Detects and carries the terminal open position.
...
def _run_backtrader_engine(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    (ETHUSDT - SOLUSDT * ratio_at_t0), normalized so 1 unit ≈ 1%
    ...
    cerebro.addstrategy(
        VPVRXsPairs30mETHSOLBacktraderStrategy,
...
def main() -> int:
    ...
    # ---- Load and align price data: ETH 15m and SOL 15m, both resampled to 30m
    eth = _load_30m_from_15m_resample(PRICE_PATH_ETH_15M)
    ...
    print(f"  common bars (post resample): {n_prices} "
          f"({common[0]} → {common[-1]})")
    print(f"  ETHUSDT resampled 30m rows: {len(eth)} (intersection)")
    ...
    validation = {
        ...
        "note": (
            ...
            "replaying trades with cost_rt=INHOUSE_COST_RT_PAIR and carrying the "
            "detected terminal open position through the final bar. ETH and SOL "
            "close series are the in-house resamples of their 15m parquets to 30m "
            "(open=first, high=max, low=min, close=last, volume=sum)."
        ),
    }
            ...
            if len(common_ts) > 0:
                ...
                engine_validation = {
                    ...
                    "note": (
                        ...
                        "bar-MTM replay on a synthetic normalized ETH-SOL spread "
    ...
    cv_record = {
        ...
        "data_source": {
            "eth_15m_path": str(PRICE_PATH_ETH_15M),
            "sol_15m_path": str(PRICE_PATH_SOL_15M),
            "resample_rule": "30min (open=first, high=max, low=min, close=last, volume=sum)",
            "resampled_to": "30m (ETH and SOL resampled from 15m)",
            "n_30m_bars": int(n),
        ...
        "inhouse_oos_walkforward": {
            ...
            "note": (
                "from walk_forward.json (iter#82 ETH/SOL pair variant; "
                "aggregate.mean_test_sharpe/mean_test_return/worst_test_mdd "
                "used as OOS reference)"
        ...
        "notes": [
            ...
            "ETHUSDT and SOLUSDT loaded as native 15m parquets and resampled to 30m on the fly.",
            "In-house cost = 8bp pair round-trip (legacy config.json basis: 1bp fee + 1bp slip per side per leg × 4 fills); backtrader cost = 8bp pair round-trip at full schedule, calibrated 1:1.",
            "Framework replay is calibrated to in-house cost (8bp legacy config basis) so the comparison isolates equity-walk mechanics from cost-model delta.",
            "The smark-proxy 2026-07-20 DECISION normalised cost basis to 24bp for the BTC/SOL pair strategies (v3 etc.); the ETH/SOL pair's in-house equity CSV was generated before that normalisation at the local 8bp basis so we use the 8bp basis here for validation fidelity.",
            "Terminal open position detected from in-house equity curve tail (entry 2026-07-10 18:30:00 short_a_long_b) and carried through final bar, matching the freqtrade adapter precedent on this strategy.",
            f"INHOUSE_COST_RT_PAIR={INHOUSE_COST_RT_PAIR:.4f} (8bp pair round-trip, legacy config.json basis).",
            f"BACKTRADER_COST_RT_PAIR={BACKTRADER_COST_RT_PAIR:.4f} (8bp pair round-trip, full backtrader schedule, calibrated 1:1).",
            "Real backtrader engine was run on a normalized pair-spread synthetic series to verify the calibrated replay.",
            "iter#82 ETH/SOL pair variant, in-house tag=NOT-PROFITABLE (current metrics.json).",
