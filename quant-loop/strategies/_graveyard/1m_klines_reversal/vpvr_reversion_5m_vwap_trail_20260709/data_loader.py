"""Data loader for vpvr_reversion_5m_vwap_trail_20260709 (iter 65).

Resamples 1m canonical parquets to 5m bars. Writes a SHA256 manifest of the
...
Output layout (per-symbol 5m cache, this strategy owns it):
    <strategy_dir>/data/
...
def _resample(df_1m: pd.DataFrame, rule: str) -> pd.DataFrame:
    if df_1m.empty:
        return df_1m.copy()
    # Translate friendly timeframe codes (5m / 15m / 1d / 1h) to pandas freq strings.
...
def _to_pandas_freq(tf: str) -> str:
    """Map friendly timeframe strings to pandas resample frequency codes."""
    tf = tf.strip().lower()
    mapping = {
        "1m": "1min",
        "5m": "5min",
    ...
    if tf in mapping:
        return mapping[tf]
    # Already a pandas freq like "5min" / "1D" — pass through.
    return tf
