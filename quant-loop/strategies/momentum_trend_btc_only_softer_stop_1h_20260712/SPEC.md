# SPEC — momentum_trend_multi_tf_atr_scaled_v2_1h_20260712

> Strategy id: `momentum_trend_multi_tf_atr_scaled_v2_1h_20260712`
> Iteration: 87 (V11 — fix V10 catastrophic w1 with regime filter + entry-anchored hard stop)
> Author: vpvr-specialist (SMA-32971)
> Date: 2026-07-13
> Universe: BTCUSDT, ETHUSDT (USD-M spot, 1h entry + 4h trend filter)
> Direction: long + short (bidirectional)
> Status: bootstrap, paper-trade only

## 0. V11 deltas vs V10

V10 (`momentum_trend_multi_tf_atr_scaled_1h_20260712`, iter#86) had
3/4 OOS windows positive at sharpe 5+, but w1 collapsed to **-14.77**.
V11 keeps V10's signal intact and adds two single-purpose fixes:

| delta | where | what | why |
|-------|-------|------|-----|
| regime filter | entry gate | skip entry when 24h realized vol > 2.0 × 30d avg realized vol | exclude crisis regimes (Luna/FTX/COVID crash) where the 4h trend lag gives misleading signals |
| entry-anchored ATR hard stop | exit gate | close if price crosses `entry ± 2.5 × ATR(14) at entry` | cap single-trade damage to 2.5 ATR even if current-bar ATR expands mid-trade |
