"""Backtrader framework adapter for vpvr_xs_pairs_30m_funding_filter_btc_sol_v10_optimize_20260717.
...
v10 grid-search parameter set: entry_z=2.5, lookback=144,
max_hold=96, fund_thr=0.0001, 3761 trades,
...
to verify the calibrated replay per `VPVRXsPairs30mSOLV10OptimizeBacktraderStrategy`.
...
2024-07. In-house walk_forward.json IS produced for this iter (v10_optimize
...
if _HAS_BACKTRADER:
    class VPVRXsPairs30mSOLV10OptimizeBacktraderStrategy(bt.Strategy):
    ...
    class VPVRXsPairs30mSOLV10OptimizeBacktraderStrategy:  # type: ignore[no-redef]
...
def _run_backtrader_engine(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    cerebro.addstrategy(
        VPVRXsPairs30mSOLV10OptimizeBacktraderStrategy,
...
def main() -> int:
    ...
    cv_record = {
        ...
        "inhouse_oos_walkforward": {
            ...
            "note": (
                "from walk_forward.json (iter#83 v10_optimize "
        ...
        "notes": [
            ...
            "iter#83 v10 grid-optimized BTC/SOL pair variant, in-house tag=NOT-PROFITABLE (current metrics.json).",
            "v10 grid-search parameter set: entry_z=2.5, lookback=144, max_hold=96, fund_thr=0.0001 (3761 trades).",
