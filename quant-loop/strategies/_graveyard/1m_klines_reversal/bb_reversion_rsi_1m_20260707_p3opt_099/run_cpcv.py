"""CPCV for P3-OPT-099 with apples-to-apples baseline comparison.

Variant: exit_threshold (exit_rsi_mid) = 0.5  — strategy loaded from config.json.
Baseline: same as variant with exit_rsi_mid forced to 50.0 (per P3-OPT-000).
...
def main() -> int:
    ...
    print(
        f"[{datetime.now(timezone.utc).isoformat()}] P3-OPT-099 CPCV start "
    ...
    variant_block = _run_cpcv(
        variant_returns,
        df_index,
        label=f"variant exit_rsi_mid={cfg['exit']['exit_rsi_mid']}",
    )
    base_block = _run_cpcv(
        base_returns,
        df_index,
        label="baseline exit_rsi_mid=50.0",
    )
    ...
    payload = _sanitize(
        {
            ...
            "parameter": {
                "name": "exit_threshold",
                "config_field": "exit.exit_rsi_mid",
                "baseline": base_cfg["exit"]["exit_rsi_mid"],
                "variant": cfg["exit"]["exit_rsi_mid"],
            },
            "cpcv_topology": {
                "n_groups": N_GROUPS,
                "k_test": K_TEST,
    ...
    summary_lines = [
        f"{cfg['strategy']} — P3-OPT-099 single-variable sweep",
        f"exit_threshold variant={cfg['exit']['exit_rsi_mid']} baseline=50.0",
