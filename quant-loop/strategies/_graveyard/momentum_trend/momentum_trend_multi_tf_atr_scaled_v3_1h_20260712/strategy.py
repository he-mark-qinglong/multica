"""Momentum / trend-following strategy (multi-TF).
...
"equity-at-risk per 1-ATR move" goal — it makes a $100 ATR on a $60k BTC
bar mean roughly $6k of notional, which is 6x on a $100k book.

Exits (first triggered wins)
----------------------------

1. 4h trend reversal — the slope sign flips against the open direction.
2. 1h RSI cross-back — RSI(14) crosses 50 against the open direction.
3. ATR trailing stop — close crosses ``entry +/- 2.5 * ATR(14)`` (anchor
   is entry, not a ratcheting high/low).

Look-ahead discipline
---------------------

Every indicator on bar ``t`` is computed from data in ``[t-W, t-1]``
only. The 4h EMA is shifted by 1 before forward-fill, so 1h bar ``t``
uses the 4h close from at most 1 bar ago in the 4h grid (i.e. up to
4h earlier in wall-clock time, but at least one full 4h bar in the past).
"""
...
def annotate(
    ...
    out["long_entry"] = long_rsi_cross & long_trend & adx_ok & have_indicators
    out["short_entry"] = short_rsi_cross & short_trend & adx_ok & have_indicators
...
def _exit_on_bar(bar: pd.Series, direction: str, entry_price: float,
    ...
    """
    cur = float(bar["close"])
    atr_now = float(bar["atr14_1h"]) if not pd.isna(bar.get("atr14_1h", np.nan)) else atr_at_entry

    # 1. ATR trailing stop (anchor = entry).
    atr_k = cfg["exit"]["atr_trailing_k"]
    if cfg["exit"].get("use_atr_trailing", True):
        if direction == "long" and cur < entry_price - atr_k * atr_now:
            return True, f"atr_trailing<entry-{atr_k}*ATR", cur
        if direction == "short" and cur > entry_price + atr_k * atr_now:
            return True, f"atr_trailing>entry+{atr_k}*ATR", cur
