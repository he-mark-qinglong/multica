"""Run the multi-TF momentum cascade backtest per symbol + portfolio."""
...
def main() -> int:
    ...
    for sym, frames in data.items():
        ...
        hold = baseline_hold(frames["15m"], cfg_t)
        ...
        summary_rows.append({
            ...
            "span_start": frames["15m"].index[0].date().isoformat(),
            "span_end": frames["15m"].index[-1].date().isoformat(),
            "strategy": {
