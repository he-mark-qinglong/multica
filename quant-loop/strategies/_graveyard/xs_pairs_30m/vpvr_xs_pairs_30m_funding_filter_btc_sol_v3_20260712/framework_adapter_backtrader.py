"""Backtrader framework adapter for vpvr_xs_pairs_30m_funding_filter_btc_sol_v3_20260712.

Cross-validate the regularized v3 BTCUSDT/SOLUSDT 30m xs-pair z-score +
VPVR confluence + funding-blowoff-filter pair strategy (iter#85,
v3 regularized parameter set: entry_z=2.8, lookback=288,
max_hold=144, fund_thr=0.0002, 1729 trades,
...
to verify the calibrated replay per `VPVRXsPairs30mSOLV3BacktraderStrategy`.
...
2024-07. In-house walk_forward.json IS produced for this iter (v3
grid-search campaign produced walk_forward.json despite NOT-PROFITABLE tag).
...
STRATEGY = STRATEGY_DIR.name  # vpvr_xs_pairs_30m_funding_filter_btc_sol_v3_20260712
DATA_DIR = STRATEGY_DIR / "data"
RESULTS_DIR = STRATEGY_DIR / "results"
TRADES_PATH = RESULTS_DIR / "trades_A_iter85_BTCUSDT_SOLUSDT.csv"
EQUITY_PATH = RESULTS_DIR / "equity_A_iter85_BTCUSDT_SOLUSDT.csv"
...
if _HAS_BACKTRADER:
    class VPVRXsPairs30mSOLV3BacktraderStrategy(bt.Strategy):
        """Backtrader IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_btc_sol_v3_20260712.
    ...
    class VPVRXsPairs30mSOLV3BacktraderStrategy:  # type: ignore[no-redef]
...
def _run_backtrader_engine(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    cerebro.addstrategy(
        VPVRXsPairs30mSOLV3BacktraderStrategy,
...
def main() -> int:
    ...
    cv_record = {
        ...
        "inhouse_oos_walkforward": {
            ...
            "note": (
                "from walk_forward.json (iter#85 v3 "
                "grid-search parameter set; "
                "aggregate.mean_test_sharpe/mean_test_return/"
                "worst_test_mdd used as OOS reference)"
        ...
        "notes": [
            ...
            "iter#85 v3 regularized BTC/SOL pair variant, in-house tag=NOT-PROFITABLE (current metrics.json).",
            "v3 regularized parameter set: entry_z=2.8, lookback=288, max_hold=144, fund_thr=0.0002 (1729 trades).",
