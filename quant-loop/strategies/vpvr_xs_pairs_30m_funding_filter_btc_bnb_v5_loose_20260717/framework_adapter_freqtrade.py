"""Freqtrade framework adapter for vpvr_xs_pairs_30m_funding_filter_btc_bnb_v5_loose_20260717.
...
- In-house pair-zscore convention mirrors the xs_basis family:
  pair direction is `long_a_short_b` (long BTC, short BNB, pos=+1)
  or `short_a_long_b` (short BTC, long BNB, pos=-1). Trades CSV
...
- Walk-forward OOS = 3 contiguous chronological folds (over 2022-01 →
  2024-07); for this NOT-PROFITABLE strategy walk_forward.json was not
  produced, so we fall back to using the single in-house aggregated
  metrics as the OOS reference.
...
# 30m native BTCUSDT/BNBUSDT parquets (per data_loader.py):
#   BTCUSDT 30m native parquet
#   BNBUSDT 30m native parquet (no resample needed; same alignment as BTC)
...
try:
    ...
    class VPVRXsPairs30mBNBFreqtradeStrategy(IStrategy):
        """Freqtrade IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_btc_bnb_v5_loose_20260717."""
    ...
    class VPVRXsPairs30mBNBFreqtradeStrategy(IStrategy):  # type: ignore[no-redef]
        def __init__(self, config: dict) -> None:
            self.config = config
            self.position = {"direction": "flat", "entry_ts": None,
                             "entry_a": 0.0, "entry_b": 0.0, "bars_held": 0}
...
def make_oos_folds(ts_index: pd.DatetimeIndex,
    ...
    Returns list of (i0, i1) index pairs aligned to the equity series.
    """
...
def main() -> int:
    ...
    # 2022-01 → 2024-07). In-house walk_forward.json was not produced for this
    # NOT-PROFITABLE strategy, so we use the same date boundaries as the rest
    # of the xs-pair family for apples-to-apples comparison.
    n = len(eq_fw)
    fold_date_windows = [
