"""Freqtrade framework adapter for vpvr_xs_pairs_30m_funding_filter_btc_sol_regularized_20260712.

Cross-validate the in-house BTCUSDT/SOLUSDT 30m xs-pair z-score +
VPVR confluence + funding-blowoff filter pair strategy (iter#83, regularized,
PROFITABLE in-house).
...
try:
    ...
    class VPVRXsPairs30mSOLRegularizedFreqtradeStrategy(IStrategy):
        """Freqtrade IStrategy wrapper for vpvr_xs_pairs_30m_funding_filter_btc_sol_regularized_20260712."""
    ...
    class VPVRXsPairs30mSOLRegularizedFreqtradeStrategy(IStrategy):  # type: ignore[no-redef]
        def __init__(self, config: dict) -> None:
            self.config = config
            self.position = {"direction": "flat", "entry_ts": None,
                             "entry_a": 0.0, "entry_b": 0.0, "bars_held": 0}
...
def main() -> int:
    ...
    cv_record = {
        ...
        "inhouse_oos_walkforward": {
            ...
            "note": (
                "from walk_forward.json (iter#83 produced walk_forward.json)"
        ...
        "notes": [
            ...
            "iter#83 regularized BTC/SOL pair variant, in-house tag=PROFITABLE (sharpe 4.75 / total_return 70.66% / max_dd -15.39%).",
