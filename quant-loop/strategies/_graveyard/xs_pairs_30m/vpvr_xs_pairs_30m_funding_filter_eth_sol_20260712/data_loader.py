"""Data loader for vpvr_xs_pairs_30m_funding_filter_eth_sol_20260712 (iter #82).

V6 spec:
  - timeframe 30m (ETH + SOL pair, no BTC).
  - Both ETHUSDT and SOLUSDT come from 15m parquets that we resample to
    30m on the fly (no native 30m parquet for either in this workspace).
  - Funding-rate parquets for ETH and SOL are loaded alongside for the
...
def load_all(symbols):
    ...
    for sym in symbols:
        if sym in ("BTCUSDT",):
            ...
            raise SystemExit("unsupported symbol in V6: " + sym)
    return out
