...
def run_pair_backtest(df_a, df_b, cfg, pair_label, funding_a=None, funding_b=None):
    ...
    if require_funding_filter:
        ...
        # Force Series (newer pandas sometimes returns DataFrame from allow_a & allow_b)
        allow_a_s = pd.Series(allow_a.values.ravel(), index=common)
        allow_b_s = pd.Series(allow_b.values.ravel(), index=common)
        funding_allow = (allow_a_s & allow_b_s).reindex(common).fillna(True)
        funding_ema = pd.Series(ema_a.values.ravel(), index=common, name="funding_8h_ema")
    else:
    ...
    for i in range(1, n):
        ...
        if cur_pos == 0 and zi is not None and require_funding_filter and not bool(funding_allow.iloc[i]):
            cur_pos = 0
