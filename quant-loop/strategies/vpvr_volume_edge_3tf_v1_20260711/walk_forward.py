"""Walk-forward + bootstrap CI + bonferroni for vpvr_volume_edge_3tf_v1.

Slices the 1m frame index for train/test windows. The 15m and 4h frames
are sliced using fixed bar counts (15m -> 4 bars per 15m bucket,
1m -> 240 bars per 4h bucket).

Schedule: ``min_windows`` sequential non-overlapping windows of
train_bars_1m + test_bars_1m, stepped by step_bars_1m. With our default
(525600 / 52560 / 52560) we expect ~3 windows on a ~4.5y span of 1m data.
"""
...
def build_schedule(n_bars: int, cfg: dict) -> List[Tuple[WindowSlice, WindowSlice]]:
    wf = cfg["walk_forward"]
    train_n = wf["train_bars_1m"]
    test_n = wf["test_bars_1m"]
    step_n = wf["step_bars_1m"]
...
def slice_frames(data, train: WindowSlice, test: WindowSlice):
    """Slice 1m, 15m, 4h frames by 1m positional index."""
    def _15m(idx: int) -> int: return idx // 15
    def _4h(idx: int) -> int: return idx // 240
    out = {}
    for sym, frames in data.items():
        out[sym] = {
            "1m": (frames["1m"].iloc[train.start_idx:train.end_idx].copy(),
                   frames["1m"].iloc[test.start_idx:test.end_idx].copy()),
            "15m": (frames["15m"].iloc[_15m(train.start_idx):_15m(train.end_idx)].copy(),
                    frames["15m"].iloc[_15m(test.start_idx):_15m(test.end_idx)].copy()),
...
def _equity_curve(starting: float, trades, idx: pd.DatetimeIndex):
    if not trades:
        return [(d, starting) for d in idx]
    ...
    for d in idx:
        while j < len(exits) and exits[j][0] <= d:
            pnl_acc += exits[j][1]
            j += 1
        curve.append((d, starting + pnl_acc))
    return curve
...
def main() -> int:
    ...
    n_bars = len(next(iter(data.values()))["1m"])
    schedule = build_schedule(n_bars, cfg)
    print(f"Walk-forward windows: {len(schedule)} (1m bars={n_bars})")
    if not schedule:
        print("Not enough bars for walk-forward")
        return 1
    if WF_DIR.exists():
    ...
    for k, (train, test) in enumerate(schedule):
        print(f"Window {k+1}: train bars={train.n_bars}, test bars={test.n_bars}")
        ...
        for sym, frames in sliced.items():
            ...
            test_curve = _equity_curve(start, test_res.trades, frames["1m"][1].index)
    ...
    summary_payload = {
        ...
        "schedule": {
            "train_bars_1m": cfg["walk_forward"]["train_bars_1m"],
            "test_bars_1m": cfg["walk_forward"]["test_bars_1m"],
            "step_bars_1m": cfg["walk_forward"]["step_bars_1m"],
        },
