"""Momentum / trend-following strategy (multi-TF) — V11 variant.

V11 changes vs V10
------------------

V10 (iter#86) produced a catastrophic OOS window 1 (sharpe -14.77)
because trend signals kept firing during a vol-regime break (likely
the FTX/Luna crash window) and the ATR trailing stop used the
**current-bar ATR** for the threshold, which widens as the regime
cracks. V11 fixes both:

1. **Regime filter** — at every potential entry bar, compare the
   24h realized vol (1h-bar log-return stdev, annualized) against the
   30d rolling mean of the same metric. If ``rv24h > rv30d_avg * k``
   (default ``k = 2.0``), suppress the entry signal. This filters out
   crisis periods without altering the trend signal itself.
2. **Entry-anchored hard stop** — the ATR stop threshold is now
   ``+/- 2.5 * ATR(14) at entry`` (NOT the current-bar ATR). This caps
   single-trade damage at the intended 2.5 ATR distance even when
   ATR expands mid-trade.
...
* **Regime filter** (V11) — 24h realized vol vs 30d avg realized vol;
  ratio must be <= ``regime_vol_ratio_max`` (default 2.0).
...
1. **ATR hard stop** (V11 — now entry-anchored) — close crosses
   ``entry +/- 2.5 * ATR(14) at entry``. Distance is fixed at entry
...
only. The 4h EMA is shifted by 1 before forward-fill. Realized vol is
computed on bars ``[t-24, t-1]`` for rv24h and ``[t-30*24, t-1]`` for the
30d average — both strictly trailing.
"""
...
def annotate(
    ...
    Returns a copy of ``df_1h`` with these extra columns:
        ...
        adx14_1h, atr14_1h,
        rv24h, rv30d_avg, rv_ratio, regime_ok (V11),
    ...
    # V11 regime filter: 24h realized vol (annualized) vs 30d rolling mean.
    # Both windows are strictly trailing. We use log-return stdev to avoid
    # drift from simple pct_change when the price series is large.
    log_ret = np.log(out["close"] / out["close"].shift(1))
    rv24h = log_ret.rolling(window=24, min_periods=24).std() * np.sqrt(8760)
    rv30d_avg = log_ret.rolling(window=24 * 30, min_periods=24 * 30).std() * np.sqrt(8760)
    out["rv24h"] = rv24h
    out["rv30d_avg"] = rv30d_avg
    out["rv_ratio"] = rv24h / rv30d_avg.replace(0.0, np.nan)
    regime_max = float(cfg.get("regime_filter", {}).get("vol_ratio_max", 2.0))
    out["regime_ok"] = (out["rv_ratio"] <= regime_max) & out["rv_ratio"].notna()
    ...
    out["long_entry"] = (
        long_rsi_cross & long_trend & adx_ok & have_indicators & out["regime_ok"]
    )
    out["short_entry"] = (
        short_rsi_cross & short_trend & adx_ok & have_indicators & out["regime_ok"]
    )
...
def _exit_on_bar(bar: pd.Series, direction: str, entry_price: float,
    ...
    V11: ATR hard stop is now **entry-anchored** — the threshold is
    ``+/- atr_k * ATR(14) at entry`` (NOT current-bar ATR). This caps
    single-trade damage at the intended distance even when ATR
    expands mid-trade during a vol-regime break.
    """
    cur = float(bar["close"])
