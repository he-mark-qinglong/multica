"""Freqtrade framework adapter for pairs_cointegration_1d_20260709 (V1).

Wraps the 3-pair (BTC/ETH/SOL) cointegration-log-OLS z-score strategy
inside a freqtrade IStrategy contract and reports the **OOS walk-forward
Sharpe** for ship-gate 5 (G5).

If freqtrade is unavailable in the runtime, falls back to a deterministic
shim that mirrors the IStrategy contract surface used here. This is the
same fallback pattern used by vpvr_vol_breakout_2tf_v1_20260711 and the
other framework-CV adapters — adapter code is fresh, only the freqtrade
framework primitives (IStrategy signature) are imported if available.
...
class PairsCointegrationFreqtradeStrategy(IStrategy):
    """Freqtrade IStrategy wrapper for the 3-pair cointegration strategy.

    The strategy is pair-relative (BTC/ETH, BTC/SOL, ETH/SOL) so each pair
    is replayed as a separate leg; the freqtrade contract is shared.
    """
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.config = config
        self.positions: List[PairsFreqtradePosition] = []
        self.trade_log: List[dict] = []

    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        # Pairs strategy reads pre-computed z-scores / OLS residuals from
        # the canonical pipeline; no inline indicator calc here.
        return dataframe
    ...
    def _load_all_trades(self) -> List[dict]:
        ...
        for pair_csv in sorted(RESULTS_DIR.glob("trades_*_*.csv")):
...
def replay_walk_forward_oos(trades: List[dict], wf_path: Path) -> List[dict]:
    ...
    for fm in folds_meta:
        ...
        # Walk-forward windows are tz-naive date strings; trades CSV has
        # tz-aware timestamps — coerce trade ts to tz-naive UTC for
        # comparison.
...
def main() -> int:
    ...
    strategy = PairsCointegrationFreqtradeStrategy(cfg)
    ...
    # Worst (most negative) max_dd across folds
    maxdd_worst = float(min(maxdd_values)) if maxdd_values else 0.0

    # In-house metrics from B3 (full sample, for divergence reference)
    inhouse_metrics: Dict[str, float] = {}
    if METRICS_PATH.exists():
        m = json.loads(METRICS_PATH.read_text())
        agg = m.get("aggregate", m)  # some files have aggregate key
        inhouse_metrics = {
            "sharpe": float(agg.get("sharpe", 0.0)),
            "total_return_pct": float(agg.get("total_return_pct", agg.get("annualized_pct", 0.0))),
            "max_drawdown_pct": float(agg.get("max_drawdown_pct", 0.0)),
    ...
    if WF_PATH.exists():
        ...
        inhouse_window_returns = [
            float(w.get("test_return_pct", 0.0)) for w in wf.get("windows", [])
        ]
        inhouse_window_mdds = [
            float(w.get("test_max_dd_pct", 0.0)) for w in wf.get("windows", [])
        ]
    ...
    # W5 divergence computation:
    #   abs((framework - inhouse) / max(|inhouse|, eps)) * 100  (percent)
    eps = 1e-6
    ...
    out = {
        ...
        "approach": (
            ...
            "(trades_BTCUSDT_ETHUSDT.csv, trades_BTCUSDT_SOLUSDT.csv, trades_ETHUSDT_SOLUSDT.csv), "
            "PnL per fold mapped to walk_forward.json test windows, framework OOS Sharpe/return/max_dd "
            "computed via numpy across folds. Compare against inhouse OOS walk-forward mean."
        ),
    }
