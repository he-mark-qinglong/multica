"""Data loader for V4: 1d resample from canonical 1m parquets.

Returns a Dict[symbol, DataFrame] aligned to the same UTC daily index, plus a
combined panel for cross-sectional work.
"""
...
def _resample_1d(df_1m: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df_1m.resample("1D", label="left", closed="left").agg(agg).dropna(subset=["close"])
