"""Freqtrade framework adapter for vpvr_macro_calendar_4h_20260715.

Cross-validate the in-house 4h BTCUSDT VPVR-POC reversion strategy gated by
a STATIC MACRO CALENDAR OVERLAY (FOMC/CPI/NFP/PCE event dates, embedded in
the repo at `macro_calendar.py`) as a risk-on/off regime filter
(iter#75 single-symbol BTCUSDT, 4h bars, USDT-margined linear perp).

Replay notes:
  - The repo holds only perp_1m and perp_30m parquet; no perp_4h on disk.
    We resample 1m → 4h on the fly (open of first 1m bar, close of last
    1m bar in each 4h bucket, standard volume-weighted aggregator).
  - 1m data spans 2019-09-08 → 2026-07-17; the in-house trades file
    extends 2022-02-05 → 2026-07-10. Every trade's entry AND exit fall
    within the 1m data window (212 trades in CSV; replay will match all
    212 inside the 4h resampled index).
  - pnl_pct applied linearly across held 4h bars with weight =
    risk_target_pct = 0.005 (from config.json).
  - Walk-forward OOS = 5 chronological folds over the 4h replay window,
    paralleling the in-house 3-fold config walk-forward intent.
...
try:
    ...
    class V75MacroCalendar4hFreqtradeStrategy(IStrategy):
        """Freqtrade IStrategy wrapper for vpvr_macro_calendar_4h_20260715."""
        timeframe = "4h"
    ...
    class V75MacroCalendar4hFreqtradeStrategy(IStrategy):  # type: ignore[no-redef]
        def __init__(self, config: dict) -> None:
            self.config = config
            self.position = {"direction": "flat", "entry_ts": None,
                             "entry_price": 0.0, "stop": 0.0, "tp": 0.0,
...
def load_prices_4h_from_1m(path_1m: Path) -> pd.DataFrame:
    ...
    Aggregation:
      ...
      low   = min 1m low
      close = last 1m close
      volume = sum 1m volume (if column present)
    Index is the UTC bucket start (aligned to floor of 4h).
    """
    ...
    bars_4h = bars_4h.sort_index()
    return bars_4h


def replay_freqtrade_linear(prices: pd.DataFrame, trades: pd.DataFrame,
                             weight: float, start_capital: float) -> tuple[pd.Series, dict]:
    """Replay USDT-margined linear-perp trades inside freqtrade IStrategy.

    pnl_pct applied linearly across held 4h bars with weight = risk_target_pct.
    Returns (equity_series, replay_stats).
    """
...
def oos_walk_forward_splits(equity: pd.Series, n_folds: int = 5) -> List[dict]:
    """Walk-forward OOS splits over the 4h replay window."""
    n = len(equity)
    if n < n_folds * 10:
        return []
    fold_size = n // n_folds
...
def main() -> int:
    ...
    if not PRICE_PATH_1M.exists():
        print(f"ERROR: price parquet not found: {PRICE_PATH_1M}", file=sys.stderr)
        return 1
    ...
    print(f"[config] strategy={STRATEGY} iter={ITERATION} tf={TIMEFRAME} "
          f"weight={WEIGHT} cap={START_CAPITAL} "
          f"freqtrade={'yes' if _HAS_FREQTRADE else 'shim'}")
    ...
    equity, replay_stats = replay_freqtrade_linear(prices, trades, WEIGHT, START_CAPITAL)
    ...
    out = {
        ...
        "timeframe": TIMEFRAME,
        "data_source": {
            "on_disk": "perp_1m/BTCUSDT_1m.parquet",
            "resampled_to": "4h",
            "n_4h_bars": int(len(prices)),
            "span_start": str(prices.index.min()),
            "span_end": str(prices.index.max()),
        ...
        "approach": (
            "freqtrade 2026.6 IStrategy contract replay: BTCUSDT 4h USDT-margined "
            "linear perp; 1m perp parquet resampled to 4h (open=first, close=last, "
            "high=max, low=min); pnl_pct applied linearly across held 4h bars with "
            "weight 0.005 (risk_target_pct); walk-forward OOS 5 folds; "
