# SPEC — vpvr_stable_depeg_regime_4h_20260716_p3opt_091 (iter#72)

**Strategy key**: `vpvr_stable_depeg_regime_4h_20260716_p3opt_091`
**Iteration**: iter#72
...
**Variant tag**: P3-OPT-091 (MAP-P3 single-variable sweep)

## Delta vs baseline (`vpvr_stable_depeg_regime_4h_20260716`)

ONE param changed: `signal_blend_weight` introduced (not present in baseline).
All other params identical.

- `signal_blend_weight = 0.5` (NEW param)
- `momentum_lookback_bars = 20` (NEW param, used by secondary signal)

### Blend rule

Primary signal (POC reversion) and secondary signal (momentum confirmation)
are blended in `build_signals.build_signals`:

- `poc_sig in {-1, 0, +1}` — existing VPVR POC reversion signal
- `mom_sig in {-1, 0, +1}` — momentum: sign of `pct_change(momentum_lookback_bars)`
- `score = signal_blend_weight * poc_sig + (1 - signal_blend_weight) * mom_sig`
- `signal = +1 if score >= 0.5`, `-1 if score <= -0.5`, else `0`

With `signal_blend_weight = 1.0` the behaviour collapses to baseline (pure POC).
With `signal_blend_weight = 0.0` it is pure momentum.
With `signal_blend_weight = 0.5` either signal alone is sufficient to enter
(more permissive than baseline).
