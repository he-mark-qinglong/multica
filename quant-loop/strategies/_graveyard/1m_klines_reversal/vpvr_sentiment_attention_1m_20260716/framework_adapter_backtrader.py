"""Backtrader framework adapter for vpvr_sentiment_attention_1m_20260716.
...
BTCUSDT 1m USDT-margined linear perp, risk_target_pct=0.005 constant,
long+short, take-profit 1.5×ATR / hard-stop 1.0×ATR / time-stop 30 bars /
cooldown 5 bars). Replay the in-house trade log inside a backtrader-
compatible broker convention and compute the framework's view of Sharpe /
ann_total_return / max_dd from a bar-by-bar mark-to-market algorithm
using actual 1m BTCUSDT perp close prices.

Mirrors the in-house equity construction in strategy.py:
  - HELD bars (entry+1 .. exit-1): equity *= (1 + risk_target * bar_pnl)
    where bar_pnl = (close / prev_close - 1) * direction
  - EXIT bar: equity *= (1 + risk_target * net) where net = pnl_pct
    (REPLACES the held-bar return at exit; in-house applies `continue`
    to skip the held-bar code path on the exit iteration).
  - FLAT bars: equity unchanged

risk_target = config.params.risk_target_pct = 0.005 (constant per strategy).
pnl_pct from trades CSV is the in-house net (= gross - inhouse_cost_rt) at
in-house cost 10bp rt (4bp fee + 1bp slip per side). The replay preserves
...
= 14bp round trip vs in-house 10bp rt (4bp fee + 1bp slip).

Per W5 (AGENT_COLLAB_AUDIT_2026-07-12): divergence > 50% -> auto-archive
                                      divergence <= 50% -> ESCALATE-TO-SMARK.

Strategy is iter #71 single-symbol BTCUSDT, timeframe 1m, USDT-margined perp.
The in-house run produced: sharpe -7.8126 (per-symbol BTCUSDT),
ann_return_pct -0.3317% (decimal -0.003317), total_return -0.000158,
max_drawdown_pct -0.0308% (decimal -0.000308), n_trades 79, profit_factor
0.3857, win_rate 0.2025 (tag = NOT-PROFITABLE).
"""
...
# 1m BTCUSDT bars are stored directly in the perp_1m parquet
PRICE_1M_PATH = Path("/home/smark/multica/quant-loop/data/perp_1m/BTCUSDT_1m.parquet")
TRADES_PATH = RESULTS_DIR / "trades_1m_BTCUSDT.csv"
EQUITY_CSV = RESULTS_DIR / "equity_BTCUSDT.csv"
...
def replay_held_bar_mtm(prices: pd.DataFrame, trades: pd.DataFrame,
    ...
    Held bars (entry+1 .. exit-1, inclusive):
    ...
    For backtrader (14bp rt vs in-house 10bp rt), cost_delta = -(14bp - 10bp)
    = -0.0004 in pnl_pct space.
    ...
    for k, t in trades.iterrows():
        ...
        # held bars are entry+1 .. exit (inclusive); the exit bar gets the
        # override, but we still mark it so the held-bar pnl is overwritten.
        held_mask[ei + 1:xi + 1] = d
...
def main() -> int:
    ...
    if not PRICE_1M_PATH.exists():
        print(f"ERROR: 1m price parquet not found: {PRICE_1M_PATH}", file=sys.stderr)
        return 1
    ...
    span_start = summary.get("span_start") or "2023-01-01"
    span_end = summary.get("span_end") or "2023-01-18"

    fee_bps = float(params.get("fee_bps_per_fill", 4.0))
    slip_bps = float(params.get("slippage_bps_per_fill", 1.0))
    ...
    elif "total_return" in summary.get("portfolio", {}):
        ih_total_ret = float(summary["portfolio"]["total_return"])
    else:
        ih_total_ret = None
    ...
    prices = load_prices_1m(PRICE_1M_PATH)
    ...
    if n_compare >= 2:
        ...
        ih_v = ih_eq_vals[:n_compare]
        denom = np.maximum(np.abs(ih_v), 1e-9)
        diff = np.abs(rp - ih_v)
        with np.errstate(divide="ignore", invalid="ignore"):
            rel = np.where(denom > 0, diff / denom, 0.0)
        max_rel = float(np.max(rel))
        final_rel = float(abs(rp[-1] - ih_v[-1]) / max(abs(ih_v[-1]), 1e-9))
        replay_dd = float((rp / np.maximum.accumulate(rp) - 1.0).min())
        ih_dd_csv = float((ih_v / np.maximum.accumulate(ih_v) - 1.0).min())
    ...
    # ---- 2) framework replay at backtrader cost (cost_delta = -0.0004)
    ...
    print(f"[framework] sharpe(trade-formula)={fw_sharpe:.4f} "
          ...
          f"total_ret={fw_total_ret*100:.6f}% max_dd={fw_max_dd*100:.6f}% "
          f"n_fills={fw_fills}")
    ...
    if ih_total_ret is not None:
        div_total_ret = R.abs_rel_div(fw_total_ret, ih_total_ret)
    else:
        div_total_ret = 0.0  # absent in metrics.json — not a divergence source
    ...
    results = {
        ...
        "fix_note": ("mirrors the in-house equity construction in strategy.py "
                     "for vpvr_sentiment_attention_1m: held-bar MTM "
                     "(equity *= (1 + risk_target * bar_pnl) on bars entry+1..exit-1 "
                     ...
                     "Single-symbol BTCUSDT 1m USDT-margined linear perp. "
                     ...
                     "10bp rt (4bp fee + 1bp slip). Validated first by replaying "
        ...
        "approach": ("backtrader 1.9.78.123 broker convention "
                     ...
                     "is held-bar MTM (entry+1..exit-1 earn risk_target*bar_pnl) "
                     "with exit-bar override (equity *= (1 + risk_target * "
                     "(pnl_pct + cost_delta))). Single-symbol BTCUSDT 1m "
